...
| Allocate page | `DiskSpaceManager.allocatePage(mtr, ref[, PageAllocationHint])` -> `TablespaceRegistry.require` -> optional `SpaceReservationService.consumePageIfReserved` -> `SegmentPageAllocator.allocatePage(..., ExtentAllocationDirection, hintPageNo, pagesNeeded)` -> `DefaultExtentAllocationPolicy` -> `SegmentSpaceService.assignExtentToSegment` -> `FreeExtentService.acquireFreeExtent`; on no space without reservation, `PageStore.extend` then `SpaceHeaderRepository.setCurrentSizeInPages` and retry -> `MiniTransaction.appendLogicalRedo(FspPageAllocationRecord)` -> `mtr.newPage(..., PageType.ALLOCATED)` | Implemented for current FSP model；0.19b 起 allocation intent 以 `FSP_PAGE_ALLOC` 持久化并在 `PAGE_INIT(ALLOCATED)` 前进入同一 MTR batch，recovery handler 只 `ensureCapacity` 不重跑 allocator；0.19c 起 SpaceHeader/XDES/INODE/FLST 写点同时追加 `FspMetadataDeltaRecord`（兼容期仍保留同页 `PAGE_BYTES`），`freePage`/`dropSegment` fragment 释放追加 `FspPageFreeRecord`；no-hint API delegates to `PageAllocationHint.none()` and keeps fragment→segment extent→autoextend behavior；direction hint only affects new extent selection/batch count after fragment slots and existing segment extents are exhausted；`INDEX_LEAF` + UP/DOWN may acquire 2-4 extents, UNDO/non-leaf/NO_DIRECTION stays single extent；active reservation 会先消费 page quota（当前 MTR reservation 的 atomic quota，无全局账本锁，避免持 index page latch 时阻塞在 reservation lock），耗尽则在分配前抛 `SpaceReservationExceededException`；registry size snapshot is not updated after autoextend because page0 remains the size authority；autoextend 现 crash-safe：恢复期 `FSP_PAGE_ALLOC`/`PAGE_INIT` 与 `SPACE_FILE_RECONCILE` 都可把物理文件长度重对齐到 redo 恢复出的逻辑边界 |
...
| Redo replay | `RedoApplyDispatcher registry` -> `FspPageAllocationRedoHandler` / `PageRedoApplyHandler` batch sessions -> `PageStore.ensureCapacity/readPage/writePage` | Implemented physical replay path + FSP intent/metadata delta replay；0.19a registry/session 已接，0.19b 生产默认注册 FSP allocation + page handler，0.19c `FspPageAllocationRedoHandler` 同时接 free intent no-op/affectedPages，`PageRedoApplyHandler` 用同一 ReplayPage cache 重放 `PAGE_INIT`/`PAGE_BYTES`/`FspMetadataDeltaRecord` 并批末一次写回；recovery discovery is not fully wired to registry |
...
| `storage.fsp.flst` file-list primitives | `FileAddress`, `Flst`, `FlstBase`, `FlstNode` | Implemented | FSP/XDES/INODE 链表指针与 base/node 编解码；不接触文件 IO；0.19c 起 FLST base/node 写入经 `FspRedoDeltas` 追加 metadata delta，兼容期仍保留 `PAGE_BYTES` |
| `storage.fsp.header` space header | `SpaceHeaderRepository`, `SpaceHeaderSnapshot`, `SpaceHeaderRawCodec`, `SpaceHeaderPhysical` | Implemented | page0 header 读写与 raw metadata 加载；`initialize` 盖 page0 FSP_HDR FilePageHeader 信封头；layout 常量供 extent/lifecycle codec 共享；0.19c 起 space header 字段写入追加 `FspMetadataDeltaRecord`，lifecycle truncate marker 仍只走兼容 `PAGE_BYTES` |
...
| `storage.fsp.extent` extent management | `ExtentDescriptorRepository`, `ExtentState`, `FreeExtentService`, `ExtentAllocationPolicy`, `ExtentAllocationDirection`, `ExtentAllocationRequest` | Implemented | XDES state/owner/bitmap + 全局 FREE/FREE_FRAG/FULL_FRAG 分配；不打开文件；direction hint 在已材料化 FREE 链中选 UP/DOWN 最近候选，UP 可推进 freeLimit 材料化右侧候选，NO_DIRECTION 保持链头语义；0.19c 起 XDES field/bitmap 写入追加 metadata delta |
...
flowchart TD
  ...
  Collector -->|recordLogical| FPAR["FspPageAllocationRecord"]
...
| Flow | Current production chain | Current state |
| --- | --- | --- |
| Redo collect (MTR) | `MtrRedoCollector.onWrite` -> `new PageBytesRecord(pageId, offset, bytes)` + local `MtrRedoEntry(record, currentCategory)`; `MtrRedoCollector.recordInit` -> `new PageInitRecord(pageId, pageType)` + `MtrRedoCategory.PAGE_INIT`; `MiniTransaction.appendLogicalRedo` -> `MtrRedoCollector.recordLogical` -> e.g. `FspPageAllocationRecord` / `FspMetadataDeltaRecord` / `FspPageFreeRecord` + `FSP_METADATA_BYTES` | Implemented; persisted redo record types: `PAGE_INIT`, `PAGE_BYTES`, `FSP_PAGE_ALLOC`, `FSP_METADATA_DELTA`, `FSP_PAGE_FREE`；classification remains MTR-local diagnostics, except logical records themselves are persisted；FSP metadata 兼容期仍同时产生 `PAGE_BYTES` |
...
| Redo replay (recovery) | `StorageEngine.open(existing)` -> recovery-open system UNDO + configured data spaces -> `CrashRecoveryService.recover` -> `RedoApplyDispatcher.pageDispatcher()`（registry 默认注册 `FspPageAllocationRedoHandler` + `PageRedoApplyHandler`）-> checkpoint-aware replay -> `RedoLogManager.restoreRecoveredBoundary(recoveredTo)` -> optional `UndoTablespaceRecoveryParticipant.resumeAfterRedo(recoveredTo)` -> `SPACE_FILE_RECONCILE` -> open traffic | Implemented production path for explicitly configured spaces；0.19a 起 dispatcher 已支持多 handler batch session，0.19b 起 `FSP_PAGE_ALLOC` 先 `ensureCapacity`；0.19c 起 `FSP_PAGE_FREE` 由 FSP intent handler no-op 接住，`FSP_METADATA_DELTA` 与 `PAGE_INIT/PAGE_BYTES` 共用 `PageRedoApplyHandler` 的同页 batch cache，避免 page0/page2 双 handler 写回覆盖；恢复边界安装后新 redo 从 recoveredTo 连续追加，durable LSN 不倒退；无 DD/tablespace discovery |
...
| `storage.redo` recovery | `RedoRecoveryReader`, `RedoApplyDispatcher`, `RedoApplyHandler`, `RedoApplyBatchHandler`, `RedoApplyContext`, `FspPageAllocationRedoHandler`, `PageRedoApplyHandler` | Implemented; production-wired by `StorageEngine` E2 | `StorageEngine.open(existing)` constructs `pageDispatcher` + `RedoApplyContext(PageStore,pageSize)`；0.19a 起 dispatcher 是多 handler registry，0.19b 默认生产注册 FSP allocation + page handler；0.19c FSP allocation/free intent 共用 intent handler，metadata delta 由 page handler 统一 patch 并批末盖 batch end LSN；dispatcher 按原始 record 顺序调用 batch session，FORCE_SKIP 在打开 handler session 和触碰 PageStore 前基于 affected pages 过滤；只恢复已打开/显式配置的表空间 |
...
| Undo page create | `UndoPageAccess.createFirstPage`/`createChainPage` -> production `newUndoEnvelope` 先 `MiniTransaction.acquireTablespaceLease(S)` + `TablespaceRegistry.require` -> `mtr.newPage(pool, pageId, EXCLUSIVE, PageType.UNDO)` + `PageEnvelope.writeHeader` -> `UndoPage.formatFirstPage`/`formatChainPage` writes page/log header bytes via `guard.writeX` | Implemented; all writes enter MTR redo as `PAGE_BYTES`; `pageLSN` stamped by MTR at commit；四参/两参低层测试构造不注入 registry 时只做页类型守门 |
...
| `RedoRecoveryReader` / `RedoApplyDispatcher` / `RedoApplyHandler` / `RedoApplyBatchHandler` / `RedoApplyContext` / `FspPageAllocationRedoHandler` / `PageRedoApplyHandler` / `RedoApplySummary` | `StorageEngine.open(existing)` + tests | Redo replay path for configured/opened tablespaces；0.19a 多 handler registry 已接，0.19b FSP allocation handler 先 ensureCapacity，page handler 保留原始 batch end LSN；FORCE_SKIP 在进入 handler session 前按 affected `PageId` 过滤 skipped space | Add tablespace discovery and more persistent logical redo records as formats expand |
...
| FSP metadata 逻辑 redo 已有 delta，但仍与 `PAGE_BYTES` 并存 | 0.23a/0.23b 已闭合 page latch ordering、savepoint touched-page 边界、commit closed LSN 顺序与 collector 本地分类；0.19b 增加 `appendLogicalRedo` 与 `FSP_PAGE_ALLOC`；0.19c 增加 `FSP_METADATA_DELTA` 与 `FSP_PAGE_FREE`，SpaceHeader/XDES/INODE/FLST 写点会追加逻辑 delta，page handler 可在无 `PAGE_BYTES` 的合成 batch 中独立 patch 元数据页。但生产收集暂未抑制同页物理 `PAGE_BYTES` | 后续 0.19d 可做 FSP metadata `PAGE_BYTES` 去重/替代；Undo/B+Tree/Trx 逻辑 redo 仍按收益逐条新增 |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| 持久 redo 记录类型仍不覆盖 Undo/BTree/Trx | 已有 `PAGE_INIT`/`PAGE_BYTES`/`FSP_PAGE_ALLOC`/`FSP_METADATA_DELTA`/`FSP_PAGE_FREE`；FSP metadata delta 可独立 replay，但生产兼容期仍同时保留物理 `PAGE_BYTES` | FSP metadata `PAGE_BYTES` 去重/替代；继续添加 Undo/BTree/Trx 逻辑 redo 类型与对应 handler |
...
| 生产默认逻辑 handler 覆盖到 FSP intent/metadata delta，但尚未覆盖 Undo/BTree/Trx | `RedoApplyDispatcher.pageDispatcher()` 默认注册 `FspPageAllocationRedoHandler` + `PageRedoApplyHandler`；FSP allocation/free intent 与 metadata delta 已接，Undo/BTree/Trx 仍无专用逻辑 record/handler | 随 BTree/Undo/Trx 持久 record type 增加对应 handler 注册 |
