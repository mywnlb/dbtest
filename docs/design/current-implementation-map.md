...
| Type | Current caller | Why it exists | Next action |
| --- | --- | --- | --- |
| `RedoLogWriter` / `RedoLogFlusher` / `RedoLogFileRepository` / 单文件/ring repository / `RedoLogBlockCodec` / `RedoLogBlockScanner` / `RedoBatchFrameCodec` / `RedoReclaimBoundary` | `StorageEngine` + tests | Durable redo write/flush/file IO（**文件环默认** + 单文件 opt-out）；0.20b 起两种容器共用 512B LogBlock v1，ring header v2；checkpoint 驱动回收，后台 flush 与前台 capacity reservation 均已生产接线 | Add per-operation exact redo budget / richer capacity diagnostics |
...
| `RedoRecoveryReader` / `RedoRecoveryScan` / dispatcher/handler types / `TransactionStateDeltaSink` | `StorageEngine.open(existing)` + tests | repository 交付 retained 边界并校验 control/data format 与 LSN 连续性；page-local redo 重放到 PageStore，trx handler 保序交给 recovery context；FORCE_SKIP 在页 handler 前过滤 | Add tablespace discovery and B+Tree node/root finer redo；trx recovery v1 已接 |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| 0.19 逻辑 redo + trx recovery 消费、0.20b LogBlock v1 已闭合，仍缺更细预算 | page-local delta 可独立 replay；`TRX_STATE_DELTA` 合并 checkpoint baseline/page3；512B block + nested frame 双 CRC、连续 blockNo/LSN 和 control format guard 已保护物理输入 | per-operation exact redo budget、B+Tree node/root 更细结构 redo |
...
| redo 环满仍保留最终 fail-closed，预算仍是保守估算 | 默认 ring 已用 LogBlock v1 做跨文件连续扫描；0.6b 在 MTR begin 前按 ASYNC_FLUSH/SYNC_FLUSH/HARD_LIMIT 请求/等待 flush+checkpoint。若 timeout、预算低估或仍无可回收区间，继续 fail-closed，不覆盖未 checkpoint redo | 按需补更细 capacity diagnostics / per-operation exact redo budget |
