...
## SQL Point Access + Secondary Prefix Range + CREATE/DROP INDEX Session Slice
...
| Parse / bind | `DefaultSqlSession.execute` -> 单一 `SqlStatementDeadline` -> parser（DML/SELECT/CREATE INDEX/DROP INDEX/ALTER ADD-DROP INDEX）-> 普通语句走 transaction policy + metadata scope；index DDL 分别生成无 DD pin 的 `BoundCreateIndex`/`BoundDropIndex`，再执行 implicit-commit 边界 | Implemented v1；CREATE 与 DROP 各自把两种语法归一为同一 AST/command；locking SELECT 仍以 RW transaction + WRITE intent 绑定；DROP 的目标存在性/clustered 属性由独立 DDL owner 在 coordinator table X 下重验 |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `sql.parser` | lexer/token/source position/AST/`DefaultSqlParser` | Implemented v1 | 单语句 INSERT/UPDATE/DELETE、等值 SELECT、尾部 FOR SHARE/UPDATE、CREATE/DROP INDEX 各两种 grammar、BEGIN/COMMIT/ROLLBACK、SET autocommit；错误含位置；无 comparison range/其余 DDL/prepared grammar |
...
| `sql.binder` | `DefaultSqlBinder`, point/write/range bound plans, `BoundCreateIndex`, `BoundDropIndex`, coercion/metadata scopes | Implemented v1 | DD 28 类型转换、严格 JSON、transaction metadata；point/range access path保持既有能力；index DDL 只规范化对象/key/order或目标名称并输出 coordinator command，不提前选择/持有物理 binding |
| `sql.executor` + `.storage` | public values/results, `DefaultSqlExecutor`, `SqlStatementDeadline`, `SqlStorageGateway`, `SqlDdlGateway` | Implemented v1 | SQL/session 对 storage 零 import；DML/query 经普通 executor，CREATE/DROP INDEX 由 Session 在 implicit-commit 边界后走独立 DDL port；两路共享绝对 deadline |
...
| `engine.adapter` | `DefaultSqlStorageGateway`, `DefaultSqlDdlGateway`, `EngineSqlTransactionHandle` | Implemented v1 | DML/query 映射与 DDL coordinator port 分离；CREATE/DROP INDEX 使用保留 DDL owner namespace，point/range/LOB/handle/deadline/fatal 语义不变 |
...
| DROP lifecycle + DISCARD/IMPORT control-plane v1 | DROP 继续持 X lease、durable DISCARDED、flush/invalidate/close/delete；新增 DD `DISCARD_PENDING/DISCARDED/IMPORT_PENDING`、DDL log v3 auxiliary path/identity、quarantine move、page0 restore NORMAL + spaceVersion bump；启动 discovery/recovery 按 pending path 续作 | SQL ALTER TABLE 语法、完整目录 fsync 和跨设备 atomic rename 仍未接 |
