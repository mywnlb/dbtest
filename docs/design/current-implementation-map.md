...
| Secondary logical key DML check | `TableDmlService.insert/update/delete` -> `SecondaryUniqueCheckService.lockLogicalKey/check` 以 collation/prefix 规范化 logical key -> `SecondaryLogicalKeyLockKey` X（事务终态释放）；unique 再 `scanSecondaryPrefixIncludingDeleted`；UPDATE/revive 的 non-unique 检查完整 physical identity；已由聚簇 unique check + INSERT_INTENTION 证明新主键的 INSERT 调 `checkFreshInsert`（`SecondaryUniqueCheckService.java:151`），non-unique 在同 logical X lock 下跳过必然 absent 的 leaf read | Implemented；unique 约束与 UPDATE A→B→A/marked identity 语义不变；fresh non-unique INSERT 不因冗余 physical check 把冷页载入，从而可进入 Change Buffer；完整 physical key 仍由 B+Tree `physicalUnique` 保护 |
...
| DROP TABLE | schema IX/table X resolve ACTIVE/binding -> `TablePurgeBarrier.awaitUnreferenced` -> DDL log `PREPARED` -> cache version barrier + repository `DROP_PENDING` -> `DICTIONARY_COMMITTED` -> await pins -> `ChangeBufferDdlBarrier.discardSpace` -> physical DISCARDED/flush/invalidate/delete -> `ENGINE_DONE` -> repository `DROPPED` -> `COMMITTED` | Implemented；Change Buffer discard 在物理文件回收前消费旧 PageId evidence；barrier timeout 发生在物理删除前，recovery 对 ACTIVE+PREPARED 回滚、对 DROP_PENDING 重过 barrier 前滚；无 marker 的旧 pending/orphan 路径仍兼容 |
