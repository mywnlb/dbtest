...
| Statement/savepoint rollback (1.4) | `ClusteredDmlService.beginStatement(txn,index)` -> 已有 `UndoContext` 时 `RollbackService.createSavepoint`，尚未首写时 `createEmptyStatementBoundary` 铸造 service+transaction 归属的一次性能力；失败 `DmlStatementGuard.rollback` -> 逐 pointer 用独立短只读 MTR 预扫描并精确命中 `(RollPointer,UndoNo)`，再逐条短读、释放 undo latch 后应用边界之后的 undo，或携令牌预扫描到 NULL 后应用当前逻辑链全部 undo -> 成功后移动逻辑链头并释放边界；成功 `close` 只消费边界、不改 undo 链；rollback 异常后 Guard 进入失败终态并标记事务 rollback-only | Implemented as explicit storage DML API；partial rollback 不进入 ROLLING_BACK/ROLLED_BACK，不释放 slot/ReadView/row locks，不写 `TRX_STATE_DELTA(ROLLED_BACK)`；`lastUndoNo` 保持 append 高水位，后续写入不复用 undoNo；rollback-only 事务保持 ACTIVE 供 full rollback，但 `assignWriteId/prepareCommit/commit` 均拒绝；多页边界扫描不长持整条 undo page 链；SQL/session/executor 尚未自动创建/驱动 Guard，命名 SAVEPOINT 未接 |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `storage.trx` facade | `TransactionManager`, `UndoLogManager`, `RollbackService`, `ReadViewManager`, `MvccReader`, `PurgeCoordinator`, `HistoryList`, `PurgeSummary` | Implemented; production-held by `StorageEngine`; DML-wired by `ClusteredDmlService` | 生命周期门面（+`begin/finishRollback` 两阶段 T1.3d；拥有 `ReadViewManager`、commit/finishRollback 调 `release` T1.4）+ undo 写门面（`beforeX`、`onCommit` 入 history/insert-reclaim，R 1.3 写 `COMMIT_NO`）+ rollback 执行器（含 1.4 savepoint/empty statement boundary API，由 `DmlStatementGuard` 调用）+ ReadView 门面 + MVCC 一致性读 + 单线程 purge 协调器；SQL/session/autocommit/命名 SAVEPOINT 尚未接 |
...
| `storage.trx` undo context | `UndoContext`, `TransactionSavepoint`, `EmptyUndoBoundary` | Implemented; DML facade writes through `UndoLogManager` and drives boundaries through `DmlStatementGuard` | T1.3c 事务 undo 子状态：`rollbackSegmentId`/`slotId`/`insertUndoFirstPageId`/`lastUndoNo` append 高水位/`logicalLastUndoNo` 当前链头/`lastRollPointer` 当前链头指针/`savepointStack`；savepoint rollback 精确匹配指针+序号后退到捕获边界，empty-boundary rollback 要求一次性归属令牌并退到 `NONE/NULL`，两者都不倒回高水位；`updateUndoLogId`/`modifiedTables` 仍 deferred |
...
| `UndoLogManager` / `UndoContext` / `TransactionSavepoint` / `EmptyUndoBoundary` / `RollbackSegmentSlotManager` / `UndoSlotExhaustedException` (T1.3c) / `RollbackService` / `RollbackSummary` (T1.3d) | `StorageEngine` production-held; `ClusteredDmlService` writes beforeX, calls onCommit/full rollback, and returns `DmlStatementGuard` for savepoint/empty-boundary rollback | 事务 undo 写门面（+`onCommit` 入 history/insert-reclaim，T-purge）+ 事务 undo 子状态 + 精确 savepoint/一次性空 statement boundary + rseg slot 目录（+`release`+`restore`）+ rollback 执行器；formal `UNDO_ROLLBACK` stage scans page3 and uses `RollbackService.rollbackRecovered` for ACTIVE slots | Wire automatic statement abort / named SQL SAVEPOINT from session/executor；`RollbackService`/`PurgeCoordinator` release 持久化仍 deferred；SQL/session/DD 多表多索引入口仍未接 |
...
| rollback 消费 undo 已接（T1.3d/e/f + 1.4） | `RollbackService.rollback` 反向走链，每条独立 MTR：INSERT_ROW→`deleteClustered`、UPDATE_ROW→`replaceClustered` 恢复旧 image、DELETE_MARK→`setClusteredDeleteMark(false)` 取消标记还原存活（均所有权校验 dbTrxId+dbRollPtr 幂等）；走到 prev=NULL 后释放 slot，并在 `finishRollback` 前追加 `TRX_STATE_DELTA(ROLLED_BACK)` 诊断 redo。1.4 起 savepoint/empty-boundary rollback 只撤销语句边界之后的 undo，事务保持 ACTIVE，slot/ReadView/row locks/终态 trx redo 均不处理 | **恢复期 rollback 已接（R 1.2）**：`RollbackService.rollbackRecovered(firstPage, 配置索引)` 无 live Transaction，`forEachRecordWithPointer` 正向收集→反向逐条 MTR 应用；engine 恢复扫 rseg 读 undo state，ACTIVE 段回滚（单显式索引，多索引/DD/prepared 留后续）；trx state redo 目前不是恢复权威；storage DML 显式 Guard 已生产可达，SQL/session 自动调用仍缺 |
| Storage DML 显式 statement guard 已接，自动语句原子性仍缺 | MTR 仍无 content undo；`ClusteredDmlService.beginStatement` 返回 `DmlStatementGuard`：已有 undo context 先预扫描精确 savepoint，首写前持一次性 `EmptyUndoBoundary`；调用方失败时显式 `rollback()` 可撤销该语句已提交短 MTR 的 INSERT/UPDATE/DELETE_MARK，成功 `close()` 只消费运行期边界；rollback 异常后 Guard 禁止重试并把 ACTIVE 事务标为 rollback-only，防止 outcome-uncertain 结果被提交。当前 `insert/update/delete` 不会自行开启 Guard，session/executor 也未驱动其异常路径 | 在 SQL executor/session statement lifecycle 自动 begin/rollback/close，并接命名 SAVEPOINT；后续补 savepoint 后行锁精细释放和持久 rolled-back marker |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| rollback 消费者已接（T1.3d/e/f + 1.4） | `RollbackService`（trx→btree 新边）读 undo 链经 `deleteClustered`/`replaceClustered`/`setClusteredDeleteMark(false)` 撤销 INSERT/UPDATE/DELETE_MARK；完整 rollback 夹在 ROLLING_BACK 内，`DmlStatementGuard` 驱动 savepoint/empty-boundary rollback 保持 ACTIVE 并只退 `UndoContext` 逻辑链头 | 见上「rollback 消费 undo 已接」；SQL/session 自动 statement lifecycle 与命名 SAVEPOINT 接线仍缺 |
