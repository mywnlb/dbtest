...
| Redo replay | `RedoApplyDispatcher registry` -> `FspPageAllocationRedoHandler` / `PageRedoApplyHandler` / `TransactionStateRedoHandler` batch sessions -> `PageStore.ensureCapacity/readPage/writePage` for page records | Implemented physical replay path + FSP intent/metadata delta + undo/rseg metadata delta + undo payload + B+Tree structure delta + non-page trx state replay；0.19a registry/session 已接，0.19b 生产默认注册 FSP allocation + page handler，0.19c `FspPageAllocationRedoHandler` 同时接 free intent no-op/affectedPages，0.19e–0.19h `PageRedoApplyHandler` 用同一 ReplayPage cache 重放 `PAGE_INIT`/`PAGE_BYTES`/FSP metadata/undo metadata/undo payload/B+Tree page delta 并批末一次写回，0.19i `TransactionStateRedoHandler` non-page no-op 消费；recovery discovery is not fully wired to registry |
...
| MTR begin/commit | `MiniTransactionManager.begin` -> optional foreground redo reservation -> `MiniTransaction.commit` -> append physical redo records -> disable collector -> stamp all touched pages with the same batch end LSN -> `memo.releaseAll()` LIFO（page guard 先于 reservation / tablespace lease，释放时发布 dirty/pageLSN）-> `RedoLogManager.markClosed(range)` -> 返回 batch end LSN | Implemented；默认 manager 仍为内存 redo + no-op reservation；`StorageEngine` 注入 durable manager + foreground reservation throttle，等待发生在 page latch/fix/lease 前；read-only/empty MTR 的空 range 不推进 closed LSN |
...
flowchart TD
  ...
  TrxHandler -->|non-page no-op| TrxDiag["transaction state diagnostics"]
...
| Flow | Current production chain | Current state |
| --- | --- | --- |
| Redo collect (MTR) | `MtrRedoCollector.onWrite` -> `new PageBytesRecord(pageId, offset, bytes)` + local `MtrRedoEntry(record, currentCategory)`; `MtrRedoCollector.recordInit` -> `new PageInitRecord(pageId, pageType)` + `MtrRedoCategory.PAGE_INIT`; `MiniTransaction.appendLogicalRedo` -> `MtrRedoCollector.recordLogical` -> FSP/Undo/BTree/Trx logical records; `MtrRedoCollector.records()` filters physical bytes exactly covered by matching logical after-images | Implemented; persisted redo record types: `PAGE_INIT`, `PAGE_BYTES`, `FSP_PAGE_ALLOC`, `FSP_METADATA_DELTA`, `FSP_PAGE_FREE`, `UNDO_METADATA_DELTA`, `UNDO_RECORD_PAYLOAD`, `BTREE_PAGE_DELTA`, `TRX_STATE_DELTA`；classification remains MTR-local diagnostics except logical records themselves are persisted；0.19d/0.19f/0.19g/0.19h 起被 FSP metadata、undo metadata、undo payload、B+Tree page delta after-image 精确覆盖的物理 bytes 不进入持久 redo，未覆盖的信封/lifecycle/row/node bytes 仍保留 |
| Redo append (MTR commit) | `MiniTransactionManager.begin` -> optional `RedoCapacityThrottle.reserveAppendBytes(foregroundBudget)`（engine 注入；默认 no-op/0 budget）-> reservation 挂入 MTR memo -> `MiniTransaction.commit` -> `redoLogManager.append(collector.records())` -> allocates `[start,end)` LSN interval + advances `readyForWriteLsn`; disable collector -> stamp pageLSN -> `memo.releaseAll()` releases page guards then reservation / publishes dirty -> `redoLogManager.markClosed(range)` advances `closedLsn` | Implemented; default no-arg manager remains memory-mode + no-op throttle, while `StorageEngine` injects durable redo + begin-time foreground reservation；capacity 等待在 MTR 获取 page latch/buffer fix/FSP lease 前执行；reservation 是保守预算账本，不改变真实 LSN 分配；checkpoint no longer treats append as closed before dirty publish；empty/read-only MTR returns a degenerate range and does not close previous gaps |
...
| Redo replay (recovery) | `StorageEngine.open(existing)` -> recovery-open system UNDO + configured data spaces -> `CrashRecoveryService.recover` -> `RedoApplyDispatcher.pageDispatcher()`（registry 默认注册 `FspPageAllocationRedoHandler` + `PageRedoApplyHandler` + `TransactionStateRedoHandler`）-> checkpoint-aware replay -> `RedoLogManager.restoreRecoveredBoundary(recoveredTo)` -> optional `UndoTablespaceRecoveryParticipant.resumeAfterRedo(recoveredTo)` -> `SPACE_FILE_RECONCILE` -> open traffic | Implemented production path for explicitly configured spaces；0.19a 起 dispatcher 已支持多 handler batch session；0.19b/0.19c FSP allocation/free intent 由 FSP handler 承接；0.19c–0.19h `PAGE_INIT/PAGE_BYTES`、FSP metadata delta、undo metadata delta、undo payload、B+Tree page delta 共用 `PageRedoApplyHandler` 同页 batch cache；0.19i `TRX_STATE_DELTA` 由 non-page handler no-op 消费且不触发 skip predicate/PageStore IO；恢复边界安装后新 redo 从 recoveredTo 连续追加，durable LSN 不倒退；无 DD/tablespace discovery |
...
| `storage.redo` recovery | `RedoRecoveryReader`, `RedoApplyDispatcher`, `RedoApplyHandler`, `RedoApplyBatchHandler`, `RedoApplyContext`, `FspPageAllocationRedoHandler`, `PageRedoApplyHandler`, `TransactionStateRedoHandler` | Implemented; production-wired by `StorageEngine` E2 | `StorageEngine.open(existing)` constructs `pageDispatcher` + `RedoApplyContext(PageStore,pageSize)`；0.19a 起 dispatcher 是多 handler registry，0.19b 默认生产注册 FSP allocation + page handler，0.19i 加 non-page trx state handler；0.19c–0.19h FSP/undo/B+Tree page-local delta 由 page handler 统一 patch 并批末盖 batch end LSN；恢复期不重新执行 FSP/undo/B+Tree/transaction 状态机；dispatcher 按原始 record 顺序调用 batch session，FORCE_SKIP 在打开 handler session和触碰 PageStore 前基于 affected pages 过滤，non-page trx record affectedPages 为空；只恢复已打开/显式配置的表空间 |
...
| Savepoint rollback (1.4) | `RollbackService.createSavepoint(txn)` -> require ACTIVE + existing `UndoContext` -> `UndoContext.createSavepoint` 捕获 `logicalLastUndoNo/lastRollPointer`；`RollbackService.rollbackToSavepoint(txn, index, savepoint)` -> require ACTIVE + owner/context 校验 -> 从当前 `ctx.lastRollPointer` 反向读 undo -> 仅应用 `undoNo > savepoint.undoNo` 的 INSERT/UPDATE/DELETE_MARK -> 成功后 `ctx.completeRollbackToSavepoint(savepoint)` 把 `logicalLastUndoNo/lastRollPointer` 退回边界并修剪栈 | Implemented as storage-internal/test-covered API；不进入 ROLLING_BACK/ROLLED_BACK，不释放 slot/ReadView/row locks，不写 `TRX_STATE_DELTA(ROLLED_BACK)`；`lastUndoNo` 保持 append 高水位，后续写入不复用 undoNo；v1 不表达首写前空 undo 边界，尚无 `ClusteredDmlService`/session/executor production caller |
...
| Slot claim | `RollbackSegmentSlotManager.claim(firstPageId)` -> `ReentrantLock` 串行扫空 slot -> 登记 `slots[i]`、`activeCount++` -> `UndoSlotId.of(i)`；无空槽抛 `UndoSlotExhaustedException` | Implemented；**0.3**：`UndoLogManager.ensureUndoContext` 认领后**同一 MTR** 经 `writeRsegSlotAfterUndoPage` -> `RollbackSegmentHeaderRepository.writeSlot` 持久到 undo page3（redo 保护，crash-safe）——engine 注入 headerRepo 时生效，纯内存 fixture 不持久；**0.19e**：page3 header/slot 写入追加 `UndoMetadataDeltaRecord(RSEG_HEADER_FIELD/RSEG_SLOT)`，但物理 `PAGE_BYTES` 仍保留；0.23a 起该写 page3 只在 UndoLogManager 编排层使用 MTR ordering 例外（已经持有/写过 undo first page X，不能提前释放；page3 元数据不反向等待 undo page latch） |
| Slot release (T1.3d) | `RollbackSegmentSlotManager.release(slot)` -> 锁内校验已占用 -> `slots[idx]=null`、`activeCount--`（first-fit 可重认领）；由 `UndoLogManager.onCommit` / `RollbackService` / `PurgeCoordinator` 调用 | Implemented；**0.3**：纯 insert 事务 `onCommit` 释放在短 MTR 内经 `writeRsegSlotAfterUndoPage(..., firstPage=null)` 清空 page3 槽（**提交后才释放内存**，engine 路径）；**0.19e**：该 page3 清槽同样有 `UndoMetadataDeltaRecord(RSEG_SLOT)` after-image；0.23a 同样使用 UndoLogManager 局部 ordering 例外（commit MTR 已持 undo first page X 并写 COMMITTED/COMMIT_NO）；`RollbackService`/`PurgeCoordinator` 释放**暂未持久**——slot 重用时 page3 被覆盖、R 1.2 读 undo 状态判 active/committed，无正确性问题，留后续 |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `storage.trx` facade | `TransactionManager`, `UndoLogManager`, `RollbackService`, `ReadViewManager`, `MvccReader`, `PurgeCoordinator`, `HistoryList`, `PurgeSummary` | Implemented; production-held by `StorageEngine`; DML-wired by `ClusteredDmlService` | 生命周期门面（+`begin/finishRollback` 两阶段 T1.3d；拥有 `ReadViewManager`、commit/finishRollback 调 `release` T1.4）+ undo 写门面（`beforeX`、`onCommit` 入 history/insert-reclaim，R 1.3 写 `COMMIT_NO`）+ rollback 执行器（含 1.4 storage-internal savepoint rollback API）+ ReadView 门面 + MVCC 一致性读 + 单线程 purge 协调器；SQL/session/autocommit/statement savepoint 尚未接 |
...
| `storage.trx` aggregate | `Transaction`, `TransactionState`, `TransactionOptions`, `IsolationLevel`, `UndoContext`, `TransactionSavepoint`, `ReadView` | Implemented; DML facade consumes explicit transaction | 5-state FSM；T1.3c 惰性 `UndoContext`；1.4 增 `TransactionSavepoint` 与 savepoint boundary；T1.4 增 `Transaction.readView` 字段 + `ReadView` 五规则；`IsolationLevel` 驱动 RR/RC ReadView 生命周期（RU/SERIALIZABLE 拒绝） |
...
| `storage.trx` undo context | `UndoContext`, `TransactionSavepoint` | Implemented; DML facade writes it through `UndoLogManager`; savepoint API test-covered | T1.3c 事务 undo 子状态：`rollbackSegmentId`/`slotId`/`insertUndoFirstPageId`/`lastUndoNo` append 高水位/`logicalLastUndoNo` 当前链头/`lastRollPointer` 当前链头指针/`savepointStack`；rollback-to 成功只退逻辑边界，不倒回高水位；`updateUndoLogId`/`modifiedTables` 仍 deferred |
...
| Undo append | `UndoLogSegment.append(rec, keyDef, schema)` (`:109`) -> `codec.encode(rec, keyDef, schema)` (`:116`) -> `current.appendRecord(payload, undoNo)` (`:119`); on `UndoPageOverflowException` -> `growAndAppend` (`:121`) -> `firstPage.setLogRecordCount(+1)` (`:123`) + `firstPage.setLogLastUndoNo(undoNo)` -> `new RollPointer(insertFlag, current.pageId().pageNo(), off)` | Implemented；INSERT/UPDATE/DELETE_MARK payload 仍以物理 `PAGE_BYTES` redo 保护；0.19e 起 append 推进的 undo page header/log header 字段追加 `UndoMetadataDeltaRecord(UNDO_PAGE_HEADER_FIELD/UNDO_LOG_HEADER_FIELD)`，但不生成完整 `UNDO_RECORD_INSERT` 逻辑 redo；T1.3c 起 `RollPointer` 经 `UndoLogManager.beforeInsert` 返回并写入聚簇 `DB_ROLL_PTR`（不再恒 NULL） |
...
| Undo page create | `UndoPageAccess.createFirstPage`/`createChainPage` -> production `newUndoEnvelope` 先 `MiniTransaction.acquireTablespaceLease(S)` + `TablespaceRegistry.require` -> `mtr.newPage(pool, pageId, EXCLUSIVE, PageType.UNDO)` + `PageEnvelope.writeHeader` -> `UndoPage.formatFirstPage`/`formatChainPage` writes page/log header bytes via `UndoRedoDeltas`/`guard.writeX` | Implemented；header/log-header 写入同时进入物理 `PAGE_BYTES` 和 0.19e `UndoMetadataDeltaRecord`，record payload/chain log-header 清零仍保留物理 redo；`pageLSN` stamped by MTR at commit；四参/两参低层测试构造不注入 registry 时只做页类型守门 |
| Undo page open | `UndoPageAccess.openUndoPage(mtr, pageId, mode)` -> production lease+Registry require -> `mtr.getPage(pool, pageId, mode)`; page-type gate rejects non-UNDO pages | Implemented |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `storage.undo` access | `UndoLogSegmentAccess`, `UndoLogSegment`, `UndoLog`, `UndoSegmentHandle`, `UndoSpaceAllocator` / `UndoSpaceReservation` (ports) | Implemented; production-held by `StorageEngine` | `UndoLogSegmentAccess` is public MTR entry：`UndoLogManager` 经它 create/open/append；T1.4 加 `readRecordByRollPointer`（按 roll pointer 跨段直读单条 undo + 槽边界/insert 位/indexId 校验），供 `MvccReader` 构造旧版本；**R 1.2/1.3 加 `UndoLogSegment.markCommitted`/`state`/`isActive`/`isCommitted`/`committedTransactionNo`**（commit 写 `STATE_COMMITTED + COMMIT_NO`，恢复判 active/committed 并按提交序重建 history）；0.19e 起这些 undo log header 状态字段写入有 `UndoMetadataDeltaRecord` 逻辑 redo；`UndoSpaceAllocator` port inverts undo->api，0.14b 加 `reserveGrowPages` 使 grow 预留不暴露 api/fsp 类型；0.23a 起 `UndoLogSegment` 只在 grow 触碰 FSP 元页、格式化未可见新 chain 页、按 RollPointer/FIL 链读历史页时使用带理由的 MTR out-of-order scope；`UndoLogSegmentAccess` 现由 `StorageEngine` 生产持有（恢复读 state/回滚/history 重建） |
| `storage.undo` page | `UndoPageAccess`, `UndoPage`, `UndoPageLayout`, `UndoRedoDeltas` | Implemented | `UndoPageAccess` centralizes `PageType.UNDO` envelope writes and passes current MTR into `UndoPage`；生产由 `UndoLogSegmentAccess` 注入 registry 后在 open/create 时先 lease+require；`UndoPageLayout` pkg-private constants (record area start=105 after R 1.3 `COMMIT_NO`); `UndoRedoDeltas` 记录 undo/rseg metadata after-image，并在 0.19g 起写完整 undo record 槽 `UndoRecordPayloadRecord`；`UndoPage` pkg-private ctor (only `UndoPageAccess` creates) |
| `storage.undo` rseg header | `RollbackSegmentHeaderRepository`, `RollbackSegmentHeaderLayout`, `RollbackSegmentHeaderSnapshot` | Implemented; production-wired by `StorageEngine`（0.3） | undo 表空间固定 **page3** slot 目录 `format`/`writeSlot`/`read`（redo 保护，`PageType.RSEG_HEADER`）；engine fresh format、slot claim/release 持久、恢复扫描共用；0.19e 起 header fields + slot pageNo after-image 追加 `UndoMetadataDeltaRecord`；§6.3 history/cached·free segment/lastTransactionNo 富字段留后续；truncate rebuild 重格式化 page3 deferred |
...
| `RedoRecoveryReader` / `RedoApplyDispatcher` / `RedoApplyHandler` / `RedoApplyBatchHandler` / `RedoApplyContext` / `FspPageAllocationRedoHandler` / `PageRedoApplyHandler` / `TransactionStateRedoHandler` / `RedoApplySummary` | `StorageEngine.open(existing)` + tests | Redo replay path for configured/opened tablespaces；0.19a 多 handler registry 已接，0.19b FSP allocation handler 先 ensureCapacity，0.19e–0.19h page handler 覆盖 PAGE_INIT/PAGE_BYTES/FSP metadata/undo metadata/undo payload/B+Tree page delta 并保留原始 batch end LSN；0.19i trx state handler 消费 non-page record；FORCE_SKIP 在进入 handler session 前按 affected `PageId` 过滤 skipped space | Add tablespace discovery, B+Tree node/root finer redo, and formal trx recovery state-table rebuild as formats expand |
...
| `UndoLogManager` / `UndoContext` / `TransactionSavepoint` / `RollbackSegmentSlotManager` / `UndoSlotExhaustedException` (T1.3c) / `RollbackService` / `RollbackSummary` (T1.3d) | `StorageEngine` production-held; `ClusteredDmlService` writes beforeX, calls onCommit and full rollback; `TransactionSavepoint`/`rollbackToSavepoint` currently test-covered storage-internal API | 事务 undo 写门面（+`onCommit` 入 history/insert-reclaim，T-purge）+ 事务 undo 子状态 + savepoint boundary + rseg slot 目录（+`release`+`restore`）+ rollback 执行器；formal `UNDO_ROLLBACK` stage scans page3 and uses `RollbackService.rollbackRecovered` for ACTIVE slots | Wire statement abort / SQL SAVEPOINT from session/executor or DML facade；`RollbackService`/`PurgeCoordinator` release 持久化仍 deferred；SQL/session/DD 多表多索引入口仍未接 |
...
| Type | Current caller | Why it exists | Next action |
| --- | --- | --- | --- |
| `CrashRecoveryService` / `RecoverySkipPolicy` / recovery request/report/gate/progress diagnostics + `UndoTablespaceRecoveryParticipant` + `TransactionUndoRecoveryParticipant` / `TransactionUndoRecoveryResult` | `StorageEngine.open(existing)` + tests；engine exposes `recoveryState()` / `lastRecoveryReport()` / `recoveryDiagnostics()` / `EngineConfig.recoveryProgressFile()` | doublewrite stage -> redo -> undo tablespace resume -> space-file reconcile -> formal UNDO_ROLLBACK/RESUME_PURGE -> traffic open 编排；`FORCE_SKIP_CORRUPT_TABLESPACE` 已支持显式 `SpaceId` 集合：doublewrite/redo/reconcile 在物理 IO 前过滤并记录 skipped 诊断；progress journal records stage start/complete/fail in memory and appends JSONL diagnostic file | Session API 查询 gate；worker/lock recovery diagnostics；full tablespace discovery、DDL_RECOVERY、多索引/prepared transaction recovery |
...
| FSP/Undo/B+Tree/Trx 逻辑 redo 已替代或补充对应物理 bytes | 0.23a/0.23b 已闭合 page latch ordering、savepoint touched-page 边界、commit closed LSN 顺序与 collector 本地分类；0.19b–0.19i 已接 FSP allocation/metadata/free、undo metadata、完整 undo payload、B+Tree sibling-link structure delta v1、non-page trx state redo；提交视图精确过滤被 after-image 覆盖的 FSP/undo/B+Tree 物理 bytes；page handler 仍兼容旧混合 batch，trx state handler 不触碰 PageStore | page0 信封、lifecycle marker、record row bytes、B+Tree node/root 结构 bytes 等未被逻辑 delta 覆盖的物理字节仍走 `PAGE_BYTES`；per-operation exact redo budget 与 log block checksum 另列后续 |
...
| B+Tree sibling-link redo v1 已接，node/root 更细结构 redo 未迁移 | `BTreeRedoDeltas.writeSiblingLinks` 已接 root split、non-root leaf split、merge、root shrink 的 FIL prev/next 写点；`BTreePageDeltaRecord(SIBLING_LINKS)` 替代被精确覆盖的 sibling-link bytes，恢复期只 patch page-local after-image | Node pointer 区、root level/header、row bytes 仍走物理 `PAGE_BYTES`；后续按收益拆更细 B+Tree page delta |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| 0.19 逻辑 redo 主线已覆盖 FSP/Undo payload/B+Tree sibling/Trx state，仍缺 log-block 与更细预算 | 已有 `PAGE_INIT`/`PAGE_BYTES`/`FSP_PAGE_ALLOC`/`FSP_METADATA_DELTA`/`FSP_PAGE_FREE`/`UNDO_METADATA_DELTA`/`UNDO_RECORD_PAYLOAD`/`BTREE_PAGE_DELTA`/`TRX_STATE_DELTA`；FSP/undo/B+Tree page-local delta 可独立 replay，并替代被 after-image 覆盖的物理 bytes；trx state 是 non-page diagnostic redo，replay no-op | 0.20b log block header/trailer checksum、per-operation exact redo budget、B+Tree node/root 更细结构 redo、正式 trx recovery state-table rebuild |
...
| 生产默认逻辑 handler 已覆盖 0.19a–0.19i record，仍无 tablespace discovery | `RedoApplyDispatcher.pageDispatcher()` 默认注册 `FspPageAllocationRedoHandler` + `PageRedoApplyHandler` + `TransactionStateRedoHandler`；FSP allocation/free intent、FSP metadata、undo metadata、undo payload、B+Tree sibling delta、trx state 均已接；page-local delta 不重跑业务状态机，trx state 不触碰 PageStore | 接 DD/tablespace discovery；按需增加 B+Tree node/root 更细结构 handler 语义和正式 trx recovery 消费者 |
...
| rollback 消费 undo 已接（T1.3d/e/f + 1.4） | `RollbackService.rollback` 反向走链，每条独立 MTR：INSERT_ROW→`deleteClustered`、UPDATE_ROW→`replaceClustered` 恢复旧 image、DELETE_MARK→`setClusteredDeleteMark(false)` 取消标记还原存活（均所有权校验 dbTrxId+dbRollPtr 幂等）；走到 prev=NULL 后释放 slot，并在 `finishRollback` 前追加 `TRX_STATE_DELTA(ROLLED_BACK)` 诊断 redo。1.4 起 `rollbackToSavepoint` 只撤销保存点之后的 undo，事务保持 ACTIVE，slot/ReadView/row locks/终态 trx redo 均不处理 | **恢复期 rollback 已接（R 1.2）**：`RollbackService.rollbackRecovered(firstPage, 配置索引)` 无 live Transaction，`forEachRecordWithPointer` 正向收集→反向逐条 MTR 应用；engine 恢复扫 rseg 读 undo state，ACTIVE 段回滚（单显式索引，多索引/DD/prepared 留后续）；trx state redo 目前不是恢复权威；savepoint rollback 尚无 SQL/session 生产调用 |
| 失败写入 orphan undo 已有 storage 内部 savepoint rollback v1，但上层语句原子性未接 | MTR 仍无 content undo；2.1 DML facade 写入失败会回滚当前 MTR 并传播异常，已经写入的 undo 可由 `RollbackService` full rollback 幂等处理；1.4 的 `createSavepoint/rollbackToSavepoint` 可撤销已有 undo context 边界之后的 INSERT/UPDATE/DELETE_MARK，但 `ClusteredDmlService`/session/executor 尚未在语句开始自动建保存点，首写前空边界也未表达 | 接 statement guard / DML facade savepoint，使普通语句失败先 rollback-to-savepoint；后续再补首写前空边界、savepoint 后行锁精细释放和持久 rolled-back marker |
...
| Trx state diagnostic redo 已接但未重建事务表 | `UndoLogManager.onCommit` 的持久 commit MTR 会追加 `TRX_STATE_DELTA(COMMITTED)`；`RollbackService.rollback` 完成 undo 后追加 `TRX_STATE_DELTA(ROLLED_BACK)`；`TransactionStateRedoHandler` 恢复期 no-op 消费，不访问 PageStore | 正式事务恢复仍以 undo/rseg header 为权威；后续 trx recovery 切片再接 prepared/recovered-active 与状态表重建 |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| rollback 消费者已接（T1.3d/e/f + 1.4） | `RollbackService`（trx→btree 新边）读 undo 链经 `deleteClustered`/`replaceClustered`/`setClusteredDeleteMark(false)` 撤销 INSERT/UPDATE/DELETE_MARK；完整 rollback 夹在 ROLLING_BACK 内，savepoint rollback 保持 ACTIVE 并只退 `UndoContext` 逻辑链头 | 见上「rollback 消费 undo 已接」；SQL/session statement savepoint 接线仍缺 |
| slot 目录 0.3 已持久 + R 1.3 history 恢复已接；部分 release 路径仍缺 | **slot claim/onCommit-release 已持久到 undo page3 + 恢复扫描重建内存目录（0.3，engine 路径）**；0.19e/0.19f 起 page3 slot 与 undo first 页 `STATE/COMMIT_NO` 另有 `UndoMetadataDeltaRecord` 逻辑 redo，且提交视图会替代被 after-image 精确覆盖的 undo metadata `PAGE_BYTES`；0.19g 起完整 undo record 槽有 `UndoRecordPayloadRecord` 逻辑 redo；COMMITTED 段由恢复期读取 `COMMIT_NO` 重建 history 后交给 `PurgeCoordinator` boundary 判死并 `dropUndoSegment` 回收；`RollbackService`/`PurgeCoordinator` 释放仍未持久、truncate rebuild 未重格式化 page3（test-wired 路径，未 engine 触达） | 全 release 路径持久 + truncate rebuild page3 format + 多 rseg/history 链表；extern payload 另列后续 |
