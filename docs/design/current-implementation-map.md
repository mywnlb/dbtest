...
| `storage.buf` read-ahead | `BufferPool.prefetch`, `LinearReadAheadTracker`, `ReadAheadRequest`, `ReadAheadHook`, `ReadAheadService`, `ReadAheadState` | Implemented (production-wired) | 0.10a linear read-ahead：`prefetch`=free-frame-only 载入 old 不 fix 不提升（跳过驻留/无空闲帧丢弃/IO 失败回收）；`LinearReadAheadTracker`(单顺序流，同 extent 连续达 threshold→预取下一 extent，`PAGES_PER_EXTENT=64`)；`ReadAheadService`(实现 `ReadAheadHook`，前台 `recordAccess` 喂检测器+有界队列、单 worker `prefetch`)；`LruBufferPool.attachReadAheadHook`(set-once)+`getPage` 回调；`StorageEngine` 后台启停（threshold 56、门控 `backgroundFlushEnabled`）。简化：单流、free-frame-only、test-wired（无生产顺序扫描驱动）。random read-ahead / warmup / 多 instance 仍缺 |
...
| read-ahead 仅 linear、其余 0.10 特性未做 | 0.10a linear read-ahead 已落并生产接线（`prefetch` + `LinearReadAheadTracker` + `ReadAheadService` + engine 启停，threshold 56，771 tests）；简化为单顺序流、free-frame-only、test-wired（无生产顺序扫描驱动） | random read-ahead（同 extent 已有足够页时补取）；warmup dump/load（热页定位 dump+启动 prefetch）；多 instance + 专用 `PageHashTable` 分片 |
