...
| `storage.fil.meta` runtime metadata | `TablespaceRegistry`, `CachingTablespaceRegistry`, `TablespaceMetadata`, `Tablespace`, `TablespaceHandle` | Implemented for runtime admission | registry 保存当前打开视图；UNDO 与 GENERAL lifecycle 由 page0 恢复；DROP/DISCARD/IMPORT/rebuild 经 DD/physical DDL 在 X lease 下执行 drain、state/generation、invalidate 与文件切换，registry 不读取 DD |
...
| Range current-read (2.7b) | `BTreeCurrentReadService.lockRange` -> 建立单次 monotonic deadline -> 短 MTR `SplitCapableBTreeIndexService.locateRangeForCurrentRead` -> commit 释放 page latch/fix -> RC record、RR next-key+terminal gap 逐项消费 remaining -> 短 MTR重扫校验；`DefaultSqlStorageGateway` 以 256 physical rows 分页，secondary 再锁 clustered row 并复核 residual | Implemented；同一批次逐行锁、terminal gap 与 relocation 共享一个绝对预算，超时逆序释放本轮已授予锁；SQL comparison/composite/full-scan locking SELECT 与 range UPDATE/DELETE 已生产消费；range DML 在首笔 mutation 前物化全部匹配聚簇 identity，terminal gap 仍是全局边界简化，不实现长期 cursor |
| Optional-bound paged scan | `SplitCapableBTreeIndexService.scan` -> 有 lower 时 `descendSharedCrab`、无 lower 时 `descendLeftmostSharedCrab` -> sibling hand-over-hand -> open/closed/optional upper 过滤；下一批使用上一批完整 physical key exclusive continuation | Implemented；任意高度；256-row SQL 批次、4096 logical rows、16384 physical candidates 双 fail-closed 上限；`scanClusteredIncludingDeleted` 为 range MVCC 保留 delete-marked 当前版本 |
...
| `storage.btree` current-read | `BTreeCurrentReadService` point/physical-unique/range + `SecondaryUniqueCheckService` logical DML key + `SecondaryCurrentReadService` logical-prefix locking read | Implemented; production-held by `StorageEngine` | 聚簇 current-read 仍按短 MTR 定位→事务锁→重定位；SQL adapter 已生产消费 point/comparison/composite/full-scan current range，secondary 再取 clustered S/X。全部事务锁当前仍由 terminal `releaseAll` 释放；global precise gap 与 RC residual-miss 提前解锁留后续 |
...
| `storage.trx` facade | transaction/undo/finalizer/rollback, `MvccReader`, `SecondaryMvccReader`, `SecondaryCurrentReadService`, `PurgeCoordinator`, `HistoryList`, `HistoryTablePurgeBarrier` | Implemented; production-held by `StorageEngine`; DML/gateway/DD-wired | 多索引 undo/rollback、prepared phase-one/two、RU/RR/RC/SERIALIZABLE read policy、命名 savepoint、locking range、version-safe purge 与 persistent-history DROP barrier 均接入；仍无多 worker purge |
...
| Flow | Current production chain | Current state |
| --- | --- | --- |
| Recovery orchestration | `DatabaseEngine.open` -> instance lock -> catalog/manifest admission -> `FileXaRegistry.openOrCreate` -> DD discovery -> `StorageEngine.open(existing, xaRegistry decision provider)` 执行 doublewrite/redo/undo/PREPARED decision/ACTIVE rollback/purge -> `xaRegistry.completeRecoveryDecisions` -> DDL marker/SDI/orphan recovery -> clean manifest -> `PersistentXaCoordinator` + Session registry -> public OPEN | Implemented production path；PREPARING 自动裁决 rollback、DECIDED 按持久方向完成、无决议 PREPARED 阻止 OPEN；普通启动不自动 catalog rebuild。仍缺对象级 force recovery与更丰富 worker/lock diagnostics |
...
flowchart TD
  ...
  Physical -->|"DISCARD/IMPORT atomic move"| Transfer["tablespace-transfer/discarded + incoming"]
...
| Table options / column defaults | `TableOptions` 与 `ColumnDefaultDefinition` -> catalog table/column payload v2 + SDI logical payload v3 -> repository/SDI decode -> binder/ALTER/storage mapping | Implemented；comment、table default charset/collation、REQUIRED/IMPLICIT_NULL/typed CONSTANT 都是 DD 权威状态；legacy catalog 按 schema/nullable 规则确定性补齐，未知版本 fail-closed |
...
| DROP secondary index | `DROP INDEX ... ON ...` / `ALTER TABLE ... DROP INDEX ...` -> Session 先绑定纯逻辑名称并隐式提交 -> 独立 DDL owner 获取 schema IX/table X -> resolve exact non-clustered definition/binding -> purge/pin drain -> reserve ddl/version + `DROP_INDEX/PREPARED` -> `beginSecondaryIndexDrop` durable DROP descriptor -> SDI 新 aggregate durable -> DD exact remove-one-index commit -> `DICTIONARY_COMMITTED` -> cache publish -> `finishSecondaryIndexDrop` 单 MTR drop leaf/non-leaf segment + exact clear footer -> `ENGINE_DONE` -> `COMMITTED` | Implemented blocking v1；不支持 IF EXISTS、多 ALTER action、DROP PRIMARY KEY 或 online row log。DD commit 是不可回退点：旧 DD 含目标时 recovery 只清 descriptor并 ROLLED_BACK，新 DD 已删除目标时完成物理回收；`PREPARED + 新 DD` phase-loss 窗口已测试前滚 |
| Controlled DISCARD / IMPORT | SQL `ALTER TABLE ... DISCARD/IMPORT TABLESPACE` -> dedicated bound plan -> implicit commit -> DDL owner table X -> DDL log phase -> exact instance transfer path atomic move；IMPORT 再校验 page0/SDI/file identity、恢复 NORMAL 并递增 spaceVersion | Implemented v1；只允许实例内 `tablespace-transfer/discarded|incoming`，跨设备来源须先由管理员复制并 force；startup 按 DD lifecycle + marker exact path 续作，不接受任意外部路径 |
| General blocking ALTER | SQL multi-action AST -> Binder typed `BoundAlterTable` -> one implicit-commit DDL -> `DictionaryDdlService.alterTable` 在 source/rename target MDL 下顺序 stage -> metadata-only 原位 SDI，或 `REBUILD_TABLE/PREPARED` -> 256-row shadow copy + source LOB hydrate/target LOB reallocate + all-index rebuild -> force/SDI -> ENGINE_DONE；两路都在 DD commit 前建立 table publication barrier -> atomic DD binding/metadata swap -> publish exact 新 cache -> old space drop -> COMMITTED | Implemented v1；支持 ADD COLUMN FIRST/AFTER、DROP COLUMN、ADD/DROP secondary、跨 schema rename、comment/default charset/collation/convert；unique/encoding/length 失败在 DD publish 前清 shadow。DD append outcome unknown 保留 barrier/marker，禁止旧 aggregate 继续服务；recovery 以 committed old/new binding 二选一恢复旧 cache 或发布新 cache。单线程 table-X blocking，无 online row log/并行/外排/断点续作、主键或类型修改 |
| DDL log / undo marker | `DictionaryDdlService` 构造稳定 `DdlUndoMarker(ddlId,dictionaryVersion,tableId)`，index operation 保存 indexId，REBUILD 保存 newSpaceId 并以 path/auxiliaryPath 表达 old/new exact path -> `PersistentDdlLogRepository` expected-phase CAS -> catalog `DDL_LOG(7)`；恢复按 ddl id 读取非终态 | Implemented for CREATE/DROP TABLE + CREATE/DROP INDEX + DISCARD/IMPORT + REBUILD_TABLE，兼容旧 marker；identity/path/UTF-8/尾随字节与 phase transition 均 fail-closed。REBUILD v1 未另存 schema digest，target SDI 由 committed DD exact reconcile；temporary undo 未接 |
| SDI v1 | `DictionarySdiCodec.encode(ACTIVE TableDefinition)` -> `SerializedDictionaryInfoService` -> `TableDdlStorageService.writeSerializedDictionaryInfo` -> SDI write MTR(page0 root→page3 body) -> `flushThrough` -> tablespace force；startup 对每张 committed ACTIVE 表执行 exact identity/version/payload compare | Implemented；GENERAL extent0 固定 page3、`PageType.SDI=4`、`SDI1/v1` header + CRC32C；逻辑 payload v3 保存 row format、table options 与 column defaults并兼容读 v1/v2。page3 尾部 96 bytes 的 index DDL footer v2 兼容 v1；root=0/空/逻辑错配按 committed DD 重写，未知 root/action/CRC/reserved bytes或物理损坏 fail-closed；不从 SDI/descriptor 反向发布 catalog |
...
| `dd.domain/repo/tx` | immutable schema/table/index/options/default definitions, `DictionaryControlStore`, `PersistentDictionaryRepository`, `PersistentDdlLogRepository`, archive/witness | Implemented catalog/baseline + DDL log | table/column payload v2 与 SDI v3 保存 options/default；legacy derive 有测试；control/catalog 写前接 manifest witness；普通 DD 与 DDL_LOG 共享 physical store但各自忽略对方 batch |
...
| `dd.ddl/recovery` | `DictionaryDdlService`, `AlterTableCommand/Action`, `DictionaryRecoveryManifestRepository`, snapshot publisher, DDL log/recovery/discovery | Production-wired blocking v1 + catalog-loss manifest v1 | DISCARD/IMPORT 与通用 shadow REBUILD 已接；全部 DDL 在副作用前检查 manifest fence、稳定终点发布 clean。仍无 online row log/binlog、主键/类型变更、foreign/generated、marker schema digest |
...
| `storage.api.ddl` + `storage.sdi` | storage schema/default/rewrite/rebuild DTO, `TableDdlStorageService`, index descriptors, `DdlUndoMarker`, SDI repository/layout | Implemented v1 + blocking rebuild | create/drop/index/transfer + 256-row continuation rebuild、LOB 重分配、redo/force/SDI；shadow failure 返回 exact binding 供 DD cleanup；storage 只解释不可变物理 schema/identity/opaque SDI，不反向依赖 DD |
...
| Controlled tablespace DDL | `ALTER TABLE t DISCARD|IMPORT TABLESPACE` -> dedicated AST/bound plan -> `prepareDdl` -> `SqlDdlGateway.alterTablespace` -> DD operation-specific lifecycle/recovery | Implemented；必须单 action；只使用实例受控 transfer 目录，IMPORT 做 exact identity/SDI 校验 |
| General ALTER TABLE SQL | ordered multi-action AST -> `DefaultSqlBinder.bindAlterTable` 完成纯 SQL type/default/name normalization -> `BoundAlterTable` -> implicit commit -> `SqlDdlGateway.alterTable` -> `AlterTableCommand` -> DD one-shot metadata update或 shadow rebuild | Implemented blocking v1；ADD/DROP COLUMN、ADD/DROP secondary、RENAME、COMMENT、DEFAULT charset/collation、CONVERT 已接；不支持主键/type/foreign/generated/online DDL |
...
| DROP secondary index SQL | parser -> `DropIndexStatementNode` -> `DefaultSqlBinder.bindDdl` -> `BoundDropIndex` -> `SessionTransactionPolicy.prepareDdl` -> `SqlDdlGateway.dropSecondaryIndex` -> `DefaultSqlDdlGateway` -> `DictionaryDdlService.dropSecondaryIndex`；finally `resumeAfterDdl` | Implemented；支持 `DROP INDEX idx ON table` 与 `ALTER TABLE table DROP INDEX idx`，拒绝 IF EXISTS/聚簇/不存在目标；逻辑名称绑定早于 implicit commit，存在性错误由独立 table X 下重验；重启后 DD 与物理 segment 均保持删除结果 |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `sql.parser` | lexer/token/source position/AST/`DefaultSqlParser` | Implemented v1 | DML/SELECT comparison range、FOR SHARE/UPDATE、transaction/savepoint、完整 v1 XA grammar、CREATE/DROP INDEX、DISCARD/IMPORT 与通用 ALTER actions；错误含位置；无 OR/IN/LIKE、主键/type/foreign/generated/online DDL grammar |
...
| `session` | `DefaultSqlSession`, execution admission, `SessionTransactionPolicy`, registry/state/options, `session.xa` port/DTO | Implemented in-process v1 | autocommit、四隔离级别、implicit/explicit/XA transaction、命名 savepoint、DDL implicit commit、rollback-only/fatal fail-close；无 network connection/prepared statement/plan cache，XA 无 active branch migration |
...
| undo/rollback/finalizer types | `StorageEngine` + table/clustered DML + SQL gateway/savepoint + formal recovery | 双 log、named/internal savepoint、secondary/LOB inverse、逐记录 purge progress 与 batch finalization | 多 worker/multi-rseg purge；普通事务锁按设计保留到终态 |
...
| --- | --- | --- |
| 公共 DD+storage+SQL Session+XA 组合根已接 | `DatabaseEngine` 先打开 catalog/manifest/XA registry，从 DD 发现 recovery spaces；storage 以 registry 决议恢复 PREPARED，再完成 DDL/SDI reconcile 后构造 XA coordinator 与 Session | SQL v1 已含 comparison range、四隔离级别、命名 savepoint、persistent XA、受控 tablespace transfer 与 general blocking ALTER；仍缺 online DDL、主键/类型/foreign/generated ALTER、network/prepared/optimizer |
...
| B+Tree current-read 已接 point/physical-unique/range并由通用 SQL range/SERIALIZABLE 消费 | 聚簇唯一继续用 record/gap/insert-intention；显式 locking 与 SERIALIZABLE promotion 都走短定位→等锁→重定位，secondary 再锁 clustered row | global gap ref/cursor 化与 RC residual-miss 锁提前释放仍待 |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| catalog-loss admission + clean manifest/full-page scrub/显式 baseline rebuild 已接 | 普通启动 fail-closed；离线 inspect 只接受完整 clean evidence；rebuild 后回普通链验证；XA registry 与 REBUILD marker 在同一 instance lock 生命周期中先后恢复 | 不自动 catalog 恢复、不从散落 SDI 猜 schema；补 object-level force recovery 与 online DDL operation-specific phase |
...
| 版本链已被 primary/secondary point/comparison/composite/full scan 与 purge safety 消费 | UPDATE/DELETE 全量 old image + ownership 校验；secondary candidate 回表执行 view/current/residual；purge checker 在 target 前检测较新 live identity；链脱节/环/上限均 fail-closed | 更复杂 purge 调度与长期 cursor 仍待 |
...
| recovery 已接 catalog-loss、XA registry decision、DD discovery、ACTIVE/PREPARED transaction、table/index/transfer/rebuild DDL marker 与 SDI reconcile | public engine 阻止误建空 catalog；registry 未决 PREPARED、REBUILD identity 第三态、非法 root/action 都 fail-closed；committed DD 决定 shadow/old space 收敛方向 | object-level force recovery、全实例离线 scrub、online DDL row-log recovery |
