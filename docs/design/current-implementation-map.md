...
| Redo replay (recovery) | `StorageEngine.open(existing)` -> recovery-open system UNDO + configured data spaces -> `CrashRecoveryService.recover` -> `RecoveryRequest.withTransactionRecovery` 绑定 `RedoApplyDispatcher.pageDispatcher(context.deltaSink())`（registry 注册 `FspPageAllocationRedoHandler` + `PageRedoApplyHandler` + `TransactionStateRedoHandler`）-> checkpoint-aware replay -> `RedoLogManager.restoreRecoveredBoundary(recoveredTo)` -> UNDO tablespace resume -> `SPACE_FILE_RECONCILE` -> recovery table/page3 reconcile + active undo rollback -> force recovery redo/data pages -> open traffic | Implemented production path for explicitly configured spaces；0.19a 起 dispatcher 已支持多 handler batch session；0.19b/0.19c FSP allocation/free intent 由 FSP handler 承接；0.19c–0.19h `PAGE_INIT/PAGE_BYTES`、FSP metadata delta、undo metadata delta、undo payload、B+Tree page delta 共用 `PageRedoApplyHandler` 同页 batch cache；0.19i `TRX_STATE_DELTA` 不触发 skip predicate/PageStore IO；正式 existing-open 把它交给 `RecoveredTransactionTable` 作为终态/counter 高水位证据并与 page3 交叉校验，通用 dispatcher 仍可显式使用 no-op sink；恢复边界安装先于 recovery rollback 追加终态 redo，durable LSN 不倒退；无 DD/tablespace discovery |
...
| `RedoCheckpointStore` / `RedoCheckpointLabel` / `TransactionRecoveryCheckpointStore` / `TransactionRecoveryCheckpoint` | `StorageEngine` + tests | redo label 与事务高水位使用独立双槽 CRC 文件；checkpoint 严格 sidecar→label→reclaim | Add LogBlock checksum and richer checkpoint diagnostics；旧非零 checkpoint 无 sidecar 明确要求重建 |
| `RedoRecoveryReader` / dispatcher/handler types / `TransactionStateDeltaSink` | `StorageEngine.open(existing)` + tests | page-local redo 重放到 PageStore；trx handler 保序交给 recovery context，默认无参 dispatcher 保留 no-op sink；FORCE_SKIP 在页 handler 前过滤 | Add tablespace discovery and B+Tree node/root finer redo；trx recovery v1 已接 |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| 0.19 逻辑 redo + trx recovery 消费已闭合，仍缺 log-block 与更细预算 | page-local delta 可独立 replay；`TRX_STATE_DELTA` 经 sink 合并 checkpoint baseline/page3，恢复纯 insert high water 与 terminal state | 0.20b log block header/trailer checksum、per-operation exact redo budget、B+Tree node/root 更细结构 redo |
...
| 生产默认逻辑 handler 已覆盖 0.19a–0.19i record，仍无 tablespace discovery | 默认注册 FSP/page/trx handler；正式 engine 给 trx handler 注入 recovery sink，通用调用保持 no-op；page-local/trx handler 都不重跑业务状态机 | 接 DD/tablespace discovery；按需增加 B+Tree node/root 更细结构 handler |
