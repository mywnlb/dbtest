...
- 除上述允许省略注释的方法外，方法级 Javadoc 必须显式写全方法契约：用正文说明方法职责和主要逻辑；每个具有业务语义的入参都必须使用 `@param` 说明来源、含义、合法范围和特殊值语义；非 `void` 方法必须使用 `@return` 说明返回对象表达的领域结果、状态及空值语义；方法可能抛出的项目领域异常必须使用 `@throws` 说明触发条件和调用方可采取的恢复动作。禁止只复述参数名、返回类型或异常类名。
- 方法的主要逻辑注释必须说明输入如何转换为返回值或领域状态变化、保护哪些关键不变量、与哪些下游协作者交互，以及成功和失败分别留下什么可观察结果；不能用“处理数据”“执行业务”“返回结果”等占位式描述代替真实逻辑。
...
- 复杂核心方法必须使用 `1、2、3、4` 式的阶段化中文注释。只要方法包含两个及以上有先后依赖的校验、状态转换、资源获取/释放、下游调用或持久化步骤，就必须在方法 Javadoc 中按顺序列出完整数据流，并在方法体对应关键代码块前使用相同编号说明当前阶段；Javadoc 与方法体编号必须一一对应，不能只在方法头笼统描述。
- 每个编号阶段必须说明“为什么执行本阶段、读取或校验什么、修改什么领域状态、调用哪个下游、产生什么 redo/dirty/lock/IO 副作用、失败时如何释放资源或保持不变量”中与该阶段相关的内容。禁止使用“1. 参数校验”“2. 执行业务”“3. 返回结果”这类无法表达数据库语义的占位式描述。
- 阶段化注释属于实现完成条件和代码评审检查项，不是可选的文档优化。新增或修改复杂核心方法时，如果缺少类职责说明、核心字段语义、方法契约、编号数据流或关键不变量说明，应视为该实现尚未完成，必须先补齐注释再交付。
- 阶段化注释参考格式如下；实际内容必须替换为当前方法真实的数据流、锁边界和恢复语义，不能照抄示例：

```java
/**
 * 刷出脏页，并保证数据页写盘前对应 redo 已经持久化。
 *
 * <p>数据流：</p>
 * <ol>
 *     <li>读取 frame 的 page LSN 和脏页状态。</li>
 *     <li>等待 redo durable LSN 覆盖 page LSN，维护 WAL 不变量。</li>
 *     <li>复制页面快照并交给磁盘层写入，避免持有页锁执行慢 IO。</li>
 *     <li>重新校验页面版本，仅在页面未被再次修改时清除脏页状态。</li>
 * </ol>
 *
 * @param pageId 待刷出页面的稳定物理标识；必须指向当前实例已打开的 tablespace
 * @throws DatabaseRuntimeException 页面不存在、redo 无法持久化或数据页写盘失败时抛出，调用方不得据此清除脏页状态
 */
public void flushPage(PageId pageId) {
    // 1. 固定 frame，避免刷盘期间页面被淘汰。
    // ...

    // 2. 在释放 page latch 后等待 redo 持久化，防止锁顺序反转。
    // ...

    // 3. 写入页面快照；此处不允许绕过 doublewrite 协作。
    // ...

    // 4. 重新校验修改版本，避免错误清除并发产生的脏状态。
    // ...
}
```
