...
- `PurgeConfig` 默认 4 worker、16 条在途 history log 与 5 秒批次上限；`EngineConfig` 可整体替换，旧低层
  `PurgeCoordinator` 构造器使用 direct executor 保持测试和单索引兼容。
- dispatcher 通过 `HistoryList.snapshotPrefix` 冻结有界物理前缀，按 `HistoryEntry.affectedTableIds` 建立
  completion DAG；同表日志严格 FIFO，一条多表日志同时占有全部 table token，互不相关表可并行。
- 当前 task 粒度是**完整 committed history log**，不是 index/page 子任务。同一 log 内仍按
  secondary→clustered→LOB ownership/head progress 串行，避免跨越同行 `PurgeDmlRowGuard` 生命周期；更细
  index/page 分片属于后续性能扩展。
- worker 只把持久 logical head 推进到 EMPTY，不取得 history removal lease。dispatcher 收齐稳定结果后只
  finalization 从物理 head 开始的连续 READY 前缀；DEFERRED/FAILED/BLOCKED 后方即使已 EMPTY 也不得先摘链。
- timeout/close 会停止接纳并取消排队 stage，但不会用线程中断切断记录内 B+Tree/MTR；普通 close 等待 worker
  到记录边界，预算耗尽则拒绝进入后续句柄释放并报告关闭失败。
...
2. 计算 `PurgeBoundary`。
3. 从 history list 读取有界物理前缀，并在首个不满足 boundary 的节点截断。
4. 如果候选的 transaction no 不小于 purge 边界，停止本批次。
5. 解析 undo record 链，构造 purge task。
6. 当前按 affected-table token 派发完整 history log；同表依赖不占 worker 线程，异表并行。
7. worker 重新定位 delete-mark 聚簇记录。
8. 再次校验记录 `DB_TRX_ID`、delete-mark 和 purge 边界。
9. 清理二级索引项，再物理移除聚簇记录或释放 record 空间。
10. worker 逐记录持久化 logical-head progress；EMPTY 后返回 READY，不直接摘 history。
11. dispatcher 严格 finalization 连续 READY 物理前缀，原子回收 undo segment/slot 并更新 metrics。
12. dispatcher 完成稳定批次并释放 worker token/history lease/MTR 后，交给有冷却的 maintenance 判断 undo
    tablespace 是否满足 truncate 条件；零进展批次也必须允许检查，使空闲实例最终能回收文件尾部。
...
- `PurgeDriverWorker` 在每个成功 batch 后调用 `PurgeCycleMaintenance`；实现为
  `PurgeDrivenUndoTruncationScheduler`，复用 driver owner 线程，不新增后台线程。
- `UndoTruncationConfig` 默认启用、至少增长 1 extent、检查冷却 30 秒；冷却使用单调时钟，配置和 metrics
  都是进程内状态，不改变磁盘格式。
- scheduler 调用 `UndoTablespaceTruncationService.tryTruncate`，以零时长公平 X lease 竞争 lifecycle；普通访问或
  排队 owner、非空 history、active slot、reuse transition 分别返回 deferred，不阻塞用户事务，也不让 driver 失败。
- X lease 内只信任 page0 持久 initial size 与当前物理文件大小计算阈值；已有 `TRUNCATING` marker 必须忽略
  当前阈值继续既有 crash-safe 协议。v1 不持久化 `TRUNCATE_CANDIDATE`，稳定状态直接从 `ACTIVE/INACTIVE`
  写 durable `TRUNCATING` marker，避免新增恢复状态。
- recovery 与 live scheduler 共享同一 truncate service、registry、redo、flush、reuse directory 和 access
  controller。格式/owner/IO/WAL 等真实失败先发布 metrics，再传播使 driver 进入 `FAILED`。
- shutdown 在 driver 锁下与 maintenance claim 定序：`STOPPING` 先发布则跳过未开始维护；维护先 claim 则 close
  有界等待其自然完成，不能在线程已经进入 marker/物理阶段后强制中断。
...
1. `PurgeCoordinator` 冻结有界运行时前缀，并在事务系统短锁内逐条复核 creator 已非 active、low water 与所有 live ReadView 可见性。
2. `PurgeWorkerPool` 按 affected-table completion DAG 派发整 log；worker 通过 B+Tree purge-safe API 定位并清理 delete-mark，期间不持有 history transition。
3. 每条记录在 row guard 内完成 secondary/clustered，再以 progress MTR 释放 LOB ownership 并推进 logical head；取消只在下一记录边界观察。
4. 全批 worker 到达 READY/DEFERRED/BLOCKED/FAILED 稳定结果后，dispatcher 从物理 head 开始为连续 READY 日志短持有 `HistoryListMutex` 取得 removal lease。
5. finalizer 预检 page3 history/free base、head owner、removed/new-head first page；最终 MTR 原子 unlink、cache/free/drop owner 并写 redo。
6. MTR commit 后才发布 slot/reuse directory/history；失败前不移动内存 head，越过物理边界失败进入 fail-stop fence。
7. 成功 batch 释放 purge 资源后调用 cooldown maintenance；单 undo space 候选通过零等待 X lease 进入既有
   crash-safe truncate，普通竞争 deferred，真实存储失败使 driver fail-stop。
...
| purge 单 task 失败 | `PurgeWorker` | 短 MTR 已提交证据保持幂等，返回 FAILED 并阻塞同表后继；dispatcher 先 finalization 安全 READY 前缀，再传播首因/suppressed，后台 driver 进入 FAILED |
...
- `PurgeWorkerPool` 只维护有界 executor、table-token DAG 和生命周期；不读取 undo 页、不取得 history transition。
...
5. 先恢复已有 committed history；prepared UPDATE commit 才能在正确 physical tail 上追加。
6. prepared transaction 按外部 provider 决议：COMMIT 将 PREPARED UPDATE 挂 history并 drop INSERT，
   ROLLBACK 走双 head inverse后 drop全部 owner；UNRESOLVED fail-closed。
7. recovered active transaction 通过 rollback service 回滚。
8. 计算启动期 purge boundary。
9. resume purge，并恢复未完成 undo truncation。
...
| undo truncation | 单系统 undo space 由 purge batch 后的 extent 阈值/cooldown 自动触发，恢复可续作 | 多 undo space 选择；结合磁盘压力、checkpoint age、history lag 的自适应决策 |
| DDL undo | 只定义 marker 和边界 | 与 atomic DDL log 深度集成 |
| temporary undo | 预留 segment kind | 临时表模块接入后进入独立临时 undo tablespace |

当前实现落点（2026-07-22）：`storage.api.ddl.DdlUndoMarker` 已由 CREATE/DROP TABLE 的独立 catalog DDL log
持久化并参与 crash recovery，不进入普通 rollback segment/history；当前 DD sidecar 也不通过用户 row undo 更新。
`UndoLogKind.TEMPORARY` 仍只保留枚举，普通 undo create/binding/recovery 明确拒绝，直到临时表 owner/lifecycle 与
独立 temporary tablespace 存在真实生产调用链。

对象级 force recovery 落点（2026-07-21）：`UndoTargetMetadata` 现在携带
`AVAILABLE/RECOVERY_UNAVAILABLE` disposition。live rollback 与普通 purge 遇不可用目标立即失败；startup
recovery rollback 与 recovery purge 必须先完整解码当前记录、验证真实 predecessor，再只推进该链首并分别累计
`skippedUnavailableUndoCount` / `skippedUnavailablePurgeCount`。该路径不调用目标 B+Tree、Record 或 PageStore，
但仍保留 undo 链、slot/history finalization 与恢复报告的持久进度不变量；系统 undo 表空间本身绝不在排除集合中。
