...
- 在 INSERT/UPDATE/DELETE 修改前生成 undo record，并返回新的 `RollPointer`。
...
- `beforeUpdate` 必须在当前读已经获得必要行锁后调用。
- `beforeUpdate` 不能在持有 `HistoryListMutex` 时访问 B+Tree 或 Record 模块。
...
- 维护事务内 undo record 序号和最新 `RollPointer`。
- 记录 insert/update undo log 的持久位置。
...
- `lastRollPointer` 是事务回滚链入口，不等于记录版本链入口；记录版本链入口存放在聚簇记录的 `DB_ROLL_PTR`。
- savepoint 只记录 undo 边界，不复制 undo record。
...
- `HistoryCursor` 指向 purge 下一个候选 undo log。
...
- full rollback：从 `UndoContext.lastRollPointer` 反向扫描到空指针。
- statement rollback：从当前语句开始 undoNo 反向扫描到语句边界。
- savepoint rollback：从最新 undoNo 反向扫描到目标 savepoint 的 undoNo。
...
| undo page header | `segmentId`, `freeOffset`, `recordCount`, `lastUndoNo` | segment 内分配 |
| undo log header | `transactionId`, `undoLogKind`, `state`, `firstRecordOffset`, `lastRecordOffset`, `prevLog`, `nextLog` | 一个事务的一条 undo log |
...
4. `RollbackSegmentSlotManager` 分配 slot。
5. insert 操作创建 insert undo log；update/delete 操作创建 update undo log。
6. `UndoContext` 记录 slot、undo log id、last undo no 和 last roll pointer。
...
3. `UndoLogManager.beforeInsert` 追加 `INSERT_ROW` undo record。
...
4. `UndoLogManager.beforeUpdate` 读取旧 hidden columns 和 changed column before image。
5. 追加 `UPDATE_ROW` 或 `DELETE_MARK` undo record，`prevRollPointer` 指向记录当前 `DB_ROLL_PTR`。
...
2. 分配 `TransactionNo`。
3. insert undo log 标记为 `REUSABLE` 或放入 insert undo cache。
4. update undo log header 状态改为 `COMMITTED_IN_HISTORY`。
5. 在 rollback segment header 中把 update undo log 追加到 history list tail。
6. 写事务提交状态 redo 和 rseg header redo。
7. 从 active transaction table 移除事务。
8. 关闭 ReadView，释放锁。
9. 唤醒 `PurgeCoordinator`。

提交时只挂 update undo log header，不逐条扫描 undo record。purge 后续按 history list 顺序处理。
...
2. 从 `UndoContext.lastRollPointer` 开始读取最新 undo record。
...
7. 直到 `prevRollPointer=null`。
8. 释放 undo slot、事务锁和 ReadView。
9. 状态 `ROLLING_BACK -> ROLLED_BACK`。

savepoint rollback：

- 只撤销 `undoNo > savepoint.undoNo` 的 undo record。
...
| `HistoryListMutex` | history list head/tail/cursor | X | 否 | 短临界区 |
...
2. 分配 `TransactionNo`。
3. `UndoSegmentMutex` 修改 update undo log header。
4. `HistoryListMutex` 追加 history node。
5. MTR commit 事务状态和 rseg header redo。
6. 从 active transaction table 移除。
7. 关闭 ReadView。
8. 释放 LockManager 中的事务锁。
9. 唤醒 purge。

purge 顺序：

1. `PurgeCoordinator` 读取 ReadView 快照，不持有 ReadViewManager 内部锁执行 IO。
2. `HistoryListMutex` 读取候选节点并立即释放。
3. worker 通过 B+Tree purge-safe API 定位记录。
4. 按 B+Tree latch coupling 获取必要 page latch。
5. 使用 MTR 清理 delete-mark、二级索引和 undo page。
6. `HistoryListMutex` 推进 cursor。
7. 可选 `UndoTablespaceLifecycleLock X` 执行 truncate；获取前必须释放所有 page latch、history mutex 和 purge queue mutex。
...
| 方法 | 输入 | 输出 | 语义 |
| --- | --- | --- | --- |
| `assignUndoContext` | transaction | undo context | 为第一次写入的事务分配 rseg slot 和 undo log |
| `beforeInsert` | transaction, record ref, record image, mtr | roll pointer | 写 insert undo，供 rollback 删除未提交插入 |
| `beforeUpdate` | transaction, record ref, old image, changed columns, mtr | roll pointer | 写 update/delete undo，供 rollback 和 MVCC 使用 |
