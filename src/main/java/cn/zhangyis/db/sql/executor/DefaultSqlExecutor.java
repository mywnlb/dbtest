...
public final class DefaultSqlExecutor {
    ...
    public SqlExecutionResult execute(SqlTransactionHandle transaction, BoundStatement statement,
        ...
        if (transaction == null || statement == null || status == null) {
            throw new DatabaseValidationException("executor transaction/statement/status must not be null");
        }
        return switch (statement) {
            case BoundClusteredInsert insert -> {
                var outcome = storage.insert(transaction, insert);
            ...
            case BoundPrimaryPointSelect select -> {
                ...
                List<SqlRow> rows = storage.selectPoint(transaction, select).stream().toList();
