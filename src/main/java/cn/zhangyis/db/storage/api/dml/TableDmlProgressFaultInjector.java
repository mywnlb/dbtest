...
/**
 * 表级 DML secondary MTR 的包内测试故障接缝。回调只位于写 MTR 创建前或成功 commit 后，不能在页修改中间注入
 * Java 异常，否则 MiniTransaction 当前没有 content undo，无法表达安全重试边界。
 */
@FunctionalInterface
interface TableDmlProgressFaultInjector {
     ...
     *
     * @param phase     写 MTR 创建前或成功提交后的稳定边界，决定故障是否已有 redo/dirty 副作用。
