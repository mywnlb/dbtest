...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `storage.trx` facade | `TransactionManager`, `UndoLogManager`, `UndoWritePlan`, `UndoSegmentReuseDirectory`, `RollbackService`, `UndoSegmentFinalizer`, `ReadViewManager`, `MvccReader`, `PurgeCoordinator`, `HistoryList`, `PurgeSummary` | Implemented; production-held by `StorageEngine`; DML-wired by `ClusteredDmlService` | 生命周期门面 + immutable undo plan/精确 reservation/redo workload + bounded per-kind cache/persistent free FIFO 投影 + undo 写/原子终结 + rollback progress + ReadView/MVCC + 单线程 purge；SQL/session/autocommit/命名 SAVEPOINT 尚未接 |
...
flowchart TD
  ...
  Resolver --> Factory["BTreeIndexMetadataFactory"]
...
| Statement metadata lease | `DataDictionaryService.openTable` -> schema MDL(SR/SW) -> table MDL(SR/SW) -> cache single-flight load + pin -> `TableMetadataLease`；close 逆序 pin→table→schema | Implemented；DDL publish 后旧 pinned 版本可保持 stale，DROP_PENDING/DROPPED 不再由普通 lookup 打开 |
| CREATE TABLE | schema IX -> table X -> reserve object/space/ddl/version high-water -> `TableDdlStorageService.createTable` 单 MTR 创建 GENERAL/FSP、每索引 leaf/non-leaf segment+root -> redo flush/force -> repository ACTIVE commit -> cache publish | Implemented for file-per-table + exactly one clustered primary index；DD publish 报错时提交结果不确定，保留物理文件；startup 对已提交 ACTIVE 继续使用、对未提交受控命名 orphan 清理 |
...
| Index resolution | rollback/purge -> `UndoStoredRecordResolver.identity` -> `IndexMetadataResolver.resolve(tableId,indexId)` -> DD table/index/storage binding -> `BTreeIndexMetadataFactory` -> exact BTree | Implemented；`StorageEngine.configureClusteredIndex` 仅保留给低层 legacy 单索引构造 |

### Package Status

| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `dd.domain/repo/tx` | immutable schema/table/index definitions, `DictionaryControlStore`, `PersistentDictionaryRepository`, `DictionaryTransaction` | Implemented v1 | control 双槽 CRC；catalog page-aligned append frame + batch manifest；version 严格单调但允许 crash reservation gap；control 回退由 catalog high-water 校正 |
| `dd.cache/mdl/service` | `DictionaryObjectCache`, `MetadataLockManager`, `DataDictionaryService`, `TableMetadataLease` | Implemented | cache single-flight/pin/stale + DROP version barrier；MDL 六模式矩阵、FIFO、upgrade、timeout、deadlock wait graph；schema→table 锁序 |
...
| `storage.api.ddl` | storage schema DTO/binding/mapper, `TableDdlStorageService`, `BTreeIndexMetadataFactory` | Implemented v1 | physical create/drop、redo durability、DISCARDED marker；公开 DTO 隔离 DD domain，factory 供 engine adapter 使用 |
| `engine` | `DatabaseEngine`, `DictionaryIndexMetadataResolver` | Implemented public composition root | DD 先于 storage discovery，storage recovery 先于 DDL cleanup，全部成功才开放公共 facade |
...
| Type | Current caller | Why it exists | Next action |
| --- | --- | --- | --- |
| recovery service/request/gate/progress + transaction recovery context/table/snapshot/reconciler/evidence + `PersistentHistoryRecovery` | `StorageEngine.open(existing)` + public `DatabaseEngine` + tests | doublewrite→redo + trx sink→undo resume/reconcile→page3/slot/cache/free cross-check→persistent history closure/high-water→DD-resolved rollback/purge→force/open；上层再做 DDL lifecycle cleanup | Session gate diagnostics；独立 DDL_LOG、XA/PREPARED 决议、对象级 force recovery |
...
| --- | --- | --- |
| 公共 DD+storage 组合根已接；SQL/session 仍待接 | `DatabaseEngine` 先打开 control/catalog，从 DD 发现 ACTIVE/DROP_PENDING recovery spaces 并注入 `StorageEngine`，同时注入 DD-backed `IndexMetadataResolver`；storage crash recovery 成功后续作 pending drop/orphan cleanup，最后才发布 public OPEN。低层 `StorageEngine` 仍可独立构造并显式配置 recovery spaces/legacy index，供模块测试与故障隔离 | 下一步接 SQL parser/binder/executor/session；完整 DDL 还需独立 DDL_LOG、SDI、ALTER/CREATE INDEX statement、purge-aware DROP 和 XA/PREPARED 决议 |
...
| Delete/Purge/Reorganize/Update 算子已有服务根与 DD index metadata resolver，但缺 SQL/二级索引驱动 | `StorageEngine` 构造 `SplitCapableBTreeIndexService`，rollback/purge 可经 undo identity + DD resolver 定位聚簇 index；`ClusteredDmlService` 已驱动单聚簇 INSERT/UPDATE/DELETE；普通 session/executor DML 与二级索引维护未接 | 接 SQL executor/session 和多索引变更协作 |
