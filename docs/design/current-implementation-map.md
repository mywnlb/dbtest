...
flowchart TD
  ...
  Codecs --> Characters
  Characters --> Collations["BinaryCollation / AsciiCaseInsensitiveCollation"]
...
| Index key comparison | leaf `RecordComparator` / node-pointer `SearchKeyComparator` -> shared `EncodedKeyPartComparator` -> NULL order -> `KeyPrefix` -> type codec；CHAR/VARCHAR codec -> `CharacterTypeRegistry.collationFor(charset,collation)` | Implemented；0.21c 起两条链共享 nullable/prefix/ASC-DESC 规则并把结果规范为 sign；UTF8/LATIN1 的 BINARY 与各自 ASCII_CI 精确注册，错配 pair fail-closed |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `record.schema` | `TableSchema`, `ColumnType`, `IndexKeyDef`, `ColumnDef`, `KeyPartDef`, `KeyOrder`, `TypeId` | Implemented | Foundational immutable value objects; consumed by btree + undo; 13 `TypeId` (TINYINT…DATETIME)；`CharsetId` 为 UTF8/LATIN1、`CollationId` 为 BINARY/两种 charset-specific ASCII_CI，均有显式 stable id/fromStableId；既有 char/varchar factory 仍默认 UTF8+BINARY |
| `record.type` | `TypeCodecRegistry`, `CharacterTypeRegistry`, `EncodedKeyPartComparator`, `TypeCodec`, `ColumnValue`, `FieldSlice`, `FieldWriter`, codecs/collations | Implemented (subset) | `StorageEngine` 生产单点创建 `TypeCodecRegistry`；0.21c 起字符 registry 初始化后只读，严格 UTF8/ISO-8859-1 编解码且无 replacement；精确 pair 缺失拒绝；ASCII_CI 只折叠 A-Z，非 MySQL Unicode weight；数值/时间/binary 保序编码不变 |
...
| `record.page` | `RecordPage`, `RecordPageStructureValidator`, `RecordPageSearch`, `RecordPageInserter`, `RecordCursor`, `RecordComparator`, `IndexPageHeader`, `RecordRef`, `SearchKey`, `RecordPageDirectory`, `HeapSpaceManager` | Partial | Insert/search/cursor/comparator wired into btree+api；0.21a/0.21b validator 由 `IndexPageAccess` 生产接线，除 live/header/directory 外，验证 FREE 链 offset/type/length/heapNo/区间与 GARBAGE 安全上下界；允许 in-place shrink 的 counted-unlinked garbage 与 oversized reuse 的 uncounted residual，不错误要求精确相等；统一抛 `PageDirectoryCorruptedException`；仍不校验 schema-aware key 顺序，split/merge 决策继续归 btree |
...
| charset/collation 基础已接（0.21c） | UTF8/LATIN1 + BINARY/charset-specific ASCII_CI 已生产接线并稳定注册；仍无 Unicode case/accent weight、normalization、locale tailoring 或更多 charset | 后续独立增加版本化 weight table；禁止改写现有 stable id 或静默改变已建索引顺序 |
...
| prefix index 比较已接（0.13d/0.21c） | `KeyPartDef.prefixBytes>0` 经共享 `EncodedKeyPartComparator` 在 collation 前截列前 N 个编码字节，leaf/node-pointer 同序；仅 CHAR/VARCHAR/BINARY/VARBINARY 可前缀，数值列拒绝。**简化**：prefixBytes 以字节计非字符；当前 BINARY/ASCII_CI 均逐字节确定，存储仍保存整值 | 引入 Unicode weight 前先定义字符级 prefix；存储层截断键与二级索引回表另做 |
