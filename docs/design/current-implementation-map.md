...
| Rollback (consume undo, T1.3d/1.4b) | `ClusteredDmlService.rollback` -> `RollbackService.rollback(txn, clusteredIndex)` -> ACTIVE 时先逐 pointer 短 MTR 预检逻辑链，再 `txnMgr.beginRollback`；ROLLING_BACK 重试直接幂等重走 -> 每条 undo 先短只读 MTR 物化并释放 undo latch/fix，再独立 index MTR 执行 INSERT/UPDATE/DELETE_MARK 反向命令 -> 写 `TRX_STATE_DELTA(ROLLED_BACK)` 诊断 redo -> `slotManager.release` -> `txnMgr.finishRollback` -> facade 仅在事务已 ROLLED_BACK 时 `LockManager.releaseAll` | Implemented for storage DML facade；链损坏在 ACTIVE→ROLLING_BACK 前失败；apply 失败保留 ROLLING_BACK、slot 与 row locks，允许同连接重试或重启恢复，避免半回滚数据失去隔离；纯状态 `TransactionManager.rollback()` 仍供只读/未写事务 |
...
| Purge boundary + 单线程聚簇 purge (T-purge/1.4b) | `TransactionSystem.purgeLowWaterNo()`=min(live ReadView lowLimitNo)/无则 nextTransactionNo -> `PurgeCoordinator.runBatch(maxLogs)`：insert-reclaim 先 `peek`，成功 drop 后才 `poll`；committed history FIFO 取 `transactionNo<boundary` -> 短 MTR 读取 first-page persistent `UndoLogicalHead`/segment handle -> 沿 `prevRollPointer` 每条短 MTR 校验 head undoNo 严格下降、creator/index，先完整收集当前逻辑链的 DELETE_MARK tasks -> 每条独立 index MTR purge -> 全成功后 drop segment、release slot、poll history | Implemented; detached rolled-back 物理分支不再被 purge；undo latch 不跨 B+Tree 修改持有；进程内硬失败在 drop 前保留 history 队首，但 history/slot release 尚未整体持久原子化，crash 边界仍列为已知缺口 |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `storage.trx` facade | `TransactionManager`, `UndoLogManager`, `RollbackService`, `ReadViewManager`, `MvccReader`, `PurgeCoordinator`, `HistoryList`, `PurgeSummary` | Implemented; production-held by `StorageEngine`; DML-wired by `ClusteredDmlService` | 生命周期门面 + undo 写门面（append 同 MTR 发布 persistent logical head）+ 可重试 full rollback + 1.4/1.4b savepoint/empty boundary 逆操作后 marker + ReadView/MVCC + 按 persistent head 的单线程 purge；SQL/session/autocommit/命名 SAVEPOINT 尚未接 |
...
flowchart TD
  ...
  TrxUndo --> TrxWork["page3 scan -> ACTIVE: persistent head -> prev short-MTR rollback / COMMITTED history rebuild"]
...
| Flow | Current production chain | Current state |
| --- | --- | --- |
| Recovery orchestration | `StorageEngine.open(existing)` -> `DiskSpaceManager.openTablespaceForRecovery(undo + non-skipped EngineConfig.recoveryTablespaces)` -> `CrashRecoveryService.recover(request)` -> gate/doublewrite/checkpoint/redo replay/boundary install/undo truncate resume/space reconcile -> `TransactionUndoRecoveryParticipant.recoverAfterRedo`：page3 扫描 ACTIVE/COMMITTED；ACTIVE 经 `RollbackService.rollbackRecovered` 用短 MTR 读 first-page persistent `UndoLogicalHead`，再沿 `prevRollPointer` 每条短读、释放 undo 页后独立 index MTR 幂等回滚；COMMITTED 按 `COMMIT_NO` 重建 history/counter -> flush recovery 新 redo -> `PageStore.forceAll` -> open traffic/workers | Implemented production path; marker 已持久则跳过 detached physical branch，marker 落后则安全重做幂等逆操作；链首 undoNo、严格下降、segment/transaction/index 归属损坏均 fail-closed；NORMAL 仍为 9 formal enum stages，READ_ONLY_VALIDATE/FORCE_SKIP 语义不变；多索引/DD/prepared/DDL_RECOVERY 未接 |
...
flowchart TD
  ...
  Rollback -->|open / logicalHead / readRecord / updateLogicalHead| Access
...
| Flow | Current production chain | Current state |
| --- | --- | --- |
| Undo segment create | `UndoLogManager.ensureUndoContext` -> `UndoLogSegmentAccess.create(mtr, spaceId, txnId)` -> `UndoSpaceAllocator.createUndoSegment`（engine 为 `DiskSpaceUndoAllocator`）-> FSP segment/page allocation -> `UndoPageAccess.createFirstPage` -> `UndoPage.formatFirstPage` -> `UndoLogSegment(EXCLUSIVE)` | Implemented and production-reachable through `ClusteredDmlService` first write；仍是单系统 undo space/单默认 rseg |
...
| Undo record/logical-chain read | `UndoLogSegment.readRecord(rp,keyDef,schema)` -> 校验 pointer insert bit、page/segment/creator/index -> resolve/open chain page -> decode；rollback/recovery/purge 先短 MTR 读 `logicalHead()`，后续每个 `prevRollPointer` 独立短 MTR 读一条 | Implemented and production-reachable；任一 B+Tree 逆操作前均释放 undo latch/fix；`forEachRecord*` 仅保留物理诊断/测试用途，不再有 production caller |
...
| `UndoLogManager` / `UndoContext` / `TransactionSavepoint` / `EmptyUndoBoundary` / `RollbackSegmentSlotManager` / `RollbackService` / `RollbackSummary` | `StorageEngine` production-held; `ClusteredDmlService` writes beforeX, commits/full-rolls back, and returns `DmlStatementGuard` | undo append/persistent logical head、精确 savepoint/空边界 marker、可重试 full rollback、rseg slot 与 recovery rollback 均生产可达 | Wire automatic statement abort / named SQL SAVEPOINT；补 savepoint lock scope、全 release 持久化与 DD 多索引入口 |
...
| `PurgeCoordinator`(impl `PurgeTarget`) / `PurgeDriverWorker` / `HistoryList` / history value types | `StorageEngine` production-wired（配置 `clusteredIndex` 时）；formal RESUME_PURGE 重建 committed history | purge 从 persistent logical head 沿 prev pointer 短 MTR 收集当前分支；insert reclaim 与 committed history 均成功回收后才 poll 内存队列 | 多 worker、二级索引、持久 history/release crash 原子边界与 DD-driven recovery 留后续片 |
...
| rollback 消费 undo 已接（T1.3d/e/f + 1.4/1.4b） | live full rollback 先短 MTR 预检严格下降逻辑链，再进入/恢复 ROLLING_BACK；每条先释放 undo 页再独立 index MTR 幂等逆操作，诊断 redo 成功后才 release slot/finish。partial rollback 在全部逆操作后以独立 X MTR CAS 持久 `UndoLogicalHead`，再更新内存 context；marker 落后可重做、绝不领先。facade 对非终态失败保留 row locks | recovery 已从 persistent head 沿 prev pointer 短 MTR 回滚，物理 detached branch 不消费；剩余是 DD 多索引/prepared/full rollback 逐条进度持久化，trx state diagnostic redo 仍非恢复权威 |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| rollback 消费者已接（T1.3d/e/f + 1.4/1.4b） | full/partial/recovery rollback 都沿逻辑链逐 pointer 短读；partial rollback 在逆操作后持久 first-page logical head，并只在 marker commit 后退 `UndoContext`；full rollback 失败保持 slot/row locks 和可重试 ROLLING_BACK | SQL/session 自动 statement lifecycle、命名 SAVEPOINT、savepoint row-lock scope、多索引/DD 仍缺 |
...
| 单线程聚簇 purge + **后台 driver + recovery resume 已接（0.4/R 1.3/1.4b）** | purge 读取 persistent logical head 并沿 prev pointer 短 MTR 收集当前分支，rolled-back detached physical branch 不再触发物理删除；insert reclaim 的 drop 失败会保留队首。COMMITTED history 仍是恢复重建的内存 FIFO，drop segment、slot release、history poll 不是跨 crash 的单个持久原子操作 | 多 worker / 二级索引 purge / 持久 history 与 release failure-boundary / 独立 update undo log 留后续片 |
