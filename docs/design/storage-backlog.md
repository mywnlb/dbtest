...
**redo 本体 Tier 0 项（0.18–0.20）**：0.1 的后台 flusher、append/fsync 拆锁、recent_written/recent_closed 和 closedLsn 已接；剩余 0.1 是 commit durable policy。随着后台 flusher 长跑，**0.18 文件轮转** 成为必要；0.19 逻辑 redo handler 可随 0.13 btree 稳定后按需加；0.20 DurabilityPolicy + LogBlock checksum 是 commit 性能与 redo 韧性收尾。三者均 storage 内闭环，不阻塞 crash-safety 主线。
