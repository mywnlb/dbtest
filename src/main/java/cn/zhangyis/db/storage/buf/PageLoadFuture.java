...
/**
 * 单页载入的"完成信号"（设计 §7.1/§7.3）。IO owner 在 poolLock 内为某帧建立本对象并注册 LOADING 占位，出锁读盘；
 ...
 *
 * <p><b>等待者不缓存帧引用</b>：await 只表达"载入已结束"，不返回帧；等待者醒来后必须重取 poolLock、按 pageId
...
final class PageLoadFuture {
    ...
    /** IO owner 读盘失败、已在 poolLock 下回收 LOADING 占位后调用，以异常唤醒全部等待者（等待者随后重试）。 */
