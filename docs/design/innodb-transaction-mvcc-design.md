...
- `UndoRecordType`：`INSERT`、`UPDATE`、`DELETE_MARK`、`REUSE_DELETE_MARKED`、`DELETE_PURGE`。
...
唯一二级 duplicate check 对非 NULL logical key 取得归一化事务级 X 锁，等待删除/改键事务进入 COMMITTED 或
ROLLED_BACK 后重扫 including-deleted 当前前缀。它不使用旧 ReadView，也不沿候选 undo 选择历史版本：live 候选冲突，
其它主键的 delete-marked 历史不冲突；PREPARED 事务在二阶段终态前继续持锁。

RC range current-read 的非匹配释放必须基于“授锁后重定位的当前聚簇行 + Executor 最终 residual”。B+Tree 只返回
RC-only scoped candidate，不能自行判断 SQL 谓词；Gateway 证明 miss 后精确关闭该候选的 access/clustered handles。
异常或未分类候选保留到事务终态，RR/SERIALIZABLE、matching row、gap/next-key 与 logical-prefix 锁不提前释放。
...
- RC residual miss 只能通过当前调用返回的精确句柄释放，不允许按 key 或 owner 批量猜测；释放时不得持页 latch/MTR。
...
3. 唯一键检查使用当前读，必要时等待冲突事务终态并重扫当前 key；不以 ReadView/undo 判定约束。
4. key 为 ABSENT 时写 insert undo；key 为 DELETE_MARKED 时写 UPDATE log 中的 `REUSE_DELETE_MARKED` 全量旧 image。
5. 普通分支插入聚簇记录；reuse 分支校验旧隐藏列后替换同 key marked 记录并清除 delete mark。
6. 普通分支发布 secondary；reuse 分支为每个新 entry 记录 `ABSENT/DELETE_MARKED` 前态后 insert/revive。
7. MTR commit；PREPARED 继续持有 record/logical locks 到二阶段终态。
8. 普通 rollback 删除新记录；reuse rollback 恢复 marked predecessor 并反向发布 secondary/LOB ownership。
9. insert undo 提交后可释放；reuse update undo 进入 history，供旧 ReadView 与 purge 使用。
