...
| Undo write (INSERT/UPDATE/DELETE) | `ClusteredDmlService.insert/update/delete` 先调用 `UndoLogManager.planInsert/planUpdate/planDelete` -> 短只读 MTR 核对 persistent append snapshot（首写无段）-> `UndoLogSegmentAccess.planRecord/plannedNewPages` 冻结编码、inline/external、精确 reservation 与 redo workload -> facade 按组合 workload begin 业务 MTR -> `appendPlanned` 重新校验事务/内存/持久快照后 reserve；首写 `slotManager.reserveClaim` + page3 preflight + create，续写 open existing segment -> `UndoLogSegment.appendPlanned` -> 发布 root record、物理高水位和 persistent logical head -> 更新 `UndoContext` -> facade 写聚簇记录并提交同一 MTR | Implemented；external 计划把 root/grow 与 payload 页全部计入 reservation/redo admission；stale 在 reservation 前拒绝；进入物理写阶段后任何失败抛 `UndoWriteFatalException`，claim lease 保持 owner fence；undo 已写后聚簇/BTree/commit 失败同样 fail-stop；legacy `beforeX` 仅为低层兼容调用保留，不是生产 DML 链 |
...
| Atomic slot release / segment finalization | `RollbackSegmentSlotManager.beginFinalization(slot,expectedOwner)` -> `UndoSegmentFinalizer.prepare` 校验 page3/first-page identity并提交 -> 独立只读 MTR `inspectDropPlan` 读取 inode page2 并提交 -> 按 fragment/extent 规模申请预算 -> final MTR `dropUndoSegment` + `clearSlot` owner CAS + 可选 terminal trx delta -> commit -> lease `complete` 发布 FREE；purge 再摘 history | Implemented for INSERT commit、live rollback、recovery rollback、committed purge；两个只读批次均不把 latch/fix 带入写批次；FSP metadata、RSEG_SLOT clear 与 commit/rollback terminal delta 同一 redo batch；持久 history linked-list仍未实现 |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `storage.trx` facade | `TransactionManager`, `UndoLogManager`, `UndoWritePlan`, `RollbackService`, `UndoSegmentFinalizer`, `ReadViewManager`, `MvccReader`, `PurgeCoordinator`, `HistoryList`, `PurgeSummary` | Implemented; production-held by `StorageEngine`; DML-wired by `ClusteredDmlService` | 生命周期门面 + begin 前 immutable undo plan/精确 reservation+redo workload + undo 写/原子终结 + savepoint/full/recovery rollback progress + ReadView/MVCC + 按 persistent head 的单线程 purge；SQL/session/autocommit/命名 SAVEPOINT 尚未接 |
...
| Flow | Current production chain | Current state |
| --- | --- | --- |
| Undo segment create | `UndoLogManager.planX` 先形成 first-write plan -> 业务 MTR `appendFirstPlanned` -> `RollbackSegmentSlotManager.reserveClaim` -> page3 `requireSlotFree` S-latch preflight/release -> 按 `pagesToReserve` 一次 reserve -> `UndoLogSegmentAccess.create(mtr, spaceId, txnId)` -> `UndoSpaceAllocator.createUndoSegment`（engine 为 `DiskSpaceUndoAllocator`）-> FSP segment/root allocation -> `UndoPageAccess.createFirstPage`/format -> external payload/root append -> claim lease bind ACTIVE -> page3 `claimSlot` CAS -> publish `UndoContext` | Implemented and production-reachable through `ClusteredDmlService` first write；预检/plan 错误发生在分配前；claim lease 进入 physical phase 后不再自动退回 FREE；bind/claim/context 或后续聚簇写失败抛 fatal 并保留 owner fence；仍是单系统 undo space/单默认 rseg |
...
| Transaction undo write (T1.3c/1.4b/1.6) | `UndoLogManager.planInsert/planUpdate/planDelete` -> immutable `UndoWritePlan` -> DML redo admission/begin -> `appendPlanned` stale preflight + exact reserve -> `seg.appendPlanned(record, prevRollPointer=ctx.logicalHead)` -> append 同 MTR 发布 external/root/物理高水位/persistent logical head -> 更新 `UndoContext` -> return pointer | Implemented and DML production-reachable；partial rollback 后 logical predecessor 指向回退边界，physical undoNo 继续递增；stale plan 在物理写前拒绝，physical phase 后异常 fail-stop，避免 MTR rollback 不撤销 buffer content 时误重试 |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `storage.undo` access | `UndoLogSegmentAccess`, `UndoLogSegment`, `UndoAppendSnapshot`, `UndoRecordWritePlan`, `UndoStoredRecordResolver`, `UndoPayloadStorage`, `UndoLogicalHead`, `UndoLog`, `UndoSegmentHandle`, allocation ports | Implemented; production-held by `StorageEngine` | create/open/plan/append、external chain write/read、commit header、persistent logical head CAS、按 pointer 直读均生产可达；rollback/recovery/purge/MVCC 共用 resolver；`UndoLog` 单页 predecessor 与 `forEachRecord*` 仅供测试/物理诊断；单 writer/out-of-order scope 约束不变 |
| `storage.undo` page | `UndoPageAccess`, `UndoPage`, `UndoPageLayout`, `UndoPayloadPage`, `UndoPayloadPageLayout`, `UndoRedoDeltas` | Implemented | 普通 UNDO v1 flags version gate + first-page 15-byte logical pair/record area=120；external 页使用 `UNDO_PAYLOAD=9` 与 `UEP1/v1` body header；两类页均走通用 envelope/page redo，v0/未知版本 fail-closed |
...
| `storage.undo` record | `UndoRecord`, `UndoRecordCodec`, `UndoRecordType`, `UndoLogKind`, `UndoPayloadDescriptor` | Partial | `INSERT_ROW` + `UPDATE_ROW` + `DELETE_MARK` 全部编解码；旧 inline 编码不变且严格 self-framing/拒绝尾随字节；35B `0x7F/v1` descriptor 把过大完整编码引向 external 页链；混合段段头 `UndoLogKind` 非权威、记录 type 字节权威；partial 仅指目标 `UPDATE_EXTERN_REF/DDL_MARKER` 等类型尚未实现 |
...
| `UndoLogSegmentAccess` / `UndoLogSegment` / `UndoRecordWritePlan` / `UndoStoredRecordResolver` / `UndoPayloadStorage` / `UndoLogicalHead` / page/record/codec/handle/allocation ports | `UndoLogManager` + `RollbackService` + `MvccReader` + recovery/purge production paths + tests | Multi-page root + external payload、v1 page/descriptor gate、persistent logical head/CAS、统一 record resolver 与 allocation port | 已生产接线；后续只需 DD 多索引、多 rseg/undo space、cached reuse/独立 insert-update log 与格式迁移方案 |
...
| slot 目录、恢复重建与四路 atomic finalization 已接 | finalization 同批 FSP drop + page3 clear + 必要 terminal delta；external payload 与 root 同属 segment，既有 drop plan 会统一释放；恢复表用 sidecar/redo 弥补已清 slot 证据，再与仍占用 slot 交叉校验 | truncate rebuild 尚未重格式化 page3；多 rseg/history 持久链表另列后续 |
