...
**redo 本体 Tier 0 项（0.18–0.20）**：0.1 的后台 flusher、append/fsync 拆锁、recent_written/recent_closed 和 closedLsn 已接。**0.18 文件轮转已全部落地并设为引擎默认**（文件环 + checkpoint 回收 + 跨文件恢复，`EngineConfig` 默认即文件环、`withSingleFileRedo()` opt-out；残留仅环满容量分级 throttle=0.6）。**0.20a DurabilityPolicy 已落**（FLUSH/WRITE/BACKGROUND + redo 三阶段 write 原语；= 原 0.1 剩余 commit durable policy，生产事务接线待 2.1）。剩余 redo Tier 0：**0.20b LogBlock checksum**（较大格式重写，ROI 偏低，按需）、**0.19 逻辑 redo handler**（随 0.13 btree 稳定后按需加）。均 storage 内闭环，不阻塞 crash-safety 主线。
