...
| Redo redo-log | `RedoLogManager` 内存/durable 双模式、`PAGE_INIT`/`PAGE_BYTES` 编码、append+flush 同步落盘、后台 `RedoFlushWorker`、`flushedToDiskLsn` WAL gate、`RedoCheckpointStore` 两槽 fuzzy checkpoint+CRC32、`RedoRecoveryReader` checkpoint 扫描、`RedoApplyDispatcher` 物理重放、recovery boundary 连续续写、`RedoCapacityPolicy` 4 级压力 | commit durable policy、append/fsync 拆锁、recent written/closed tracker、redo 文件轮转/回收、多 apply handler/逻辑 redo、DurabilityPolicy、LogBlock checksum/header-trailer | ~45% |
...
**redo 本体 Tier 0 项（0.18–0.20）**：0.1 的后台 flusher 已接，剩余最高杠杆是拆 append/fsync 锁、recent_written/recent_closed 和 closedLsn；随着后台 flusher 长跑，**0.18 文件轮转** 成为必要；0.19 逻辑 redo handler 可随 0.13 btree 稳定后按需加；0.20 DurabilityPolicy + LogBlock checksum 是 commit 性能与 redo 韧性收尾。三者均 storage 内闭环，不阻塞 crash-safety 主线。
