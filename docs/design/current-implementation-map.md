...
| Flow | Current production chain | Current state |
| --- | --- | --- |
| Recovery orchestration | `DatabaseEngine.open` -> instance lock -> catalog/manifest admission -> `FileXaRegistry.openOrCreate` -> `DictionaryRecoveryIsolationPlanner` -> DD discovery -> `StorageEngine.open(existing, xaRegistry decision provider)` 执行 doublewrite/redo/undo/PREPARED decision/ACTIVE rollback/purge -> `xaRegistry.completeRecoveryDecisions` -> recovery-object/ordinary DDL marker、SDI/orphan recovery -> clean manifest -> `PersistentXaCoordinator` + Session registry -> public OPEN | Implemented production path；PREPARING 自动裁决 rollback、DECIDED 按持久方向完成、无决议 PREPARED 阻止 OPEN；普通启动不自动 catalog rebuild；对象级 force 已接 file-per-table v1，仍缺更丰富 worker/lock diagnostics |
...
| DDL log / undo marker | `DictionaryDdlService` 构造稳定 `DdlUndoMarker(ddlId,dictionaryVersion,tableId)`，index operation 保存 indexId，REBUILD 保存 newSpaceId，path/auxiliaryPath 保存 exact 物理 identity -> `PersistentDdlLogRepository` expected-phase CAS -> catalog `DDL_LOG(7)`；恢复按 ddl id 读取非终态 | Implemented for operation 1..10：CREATE/DROP TABLE、CREATE/DROP INDEX、普通 DISCARD/IMPORT、REBUILD 及 recovery raw DISCARD/DROP/trusted IMPORT；identity/path/UTF-8/尾随字节与 phase transition fail-closed。op8/9/10 固定 physical-first；REBUILD 尚无 schema digest，temporary undo 未接 |
| SDI v1 | `DictionarySdiCodec.encode(ACTIVE TableDefinition)` -> `SerializedDictionaryInfoService` -> `TableDdlStorageService.writeSerializedDictionaryInfo` -> SDI write MTR(page0 root→page3 body) -> `flushThrough` -> tablespace force；startup 只对 committed ACTIVE 表执行 exact compare | Implemented；GENERAL extent0 固定 page3、`PageType.SDI=4`、`SDI1/v1` header + CRC32C；逻辑 payload v3 保存 row format/options/default；`RECOVERY_UNAVAILABLE/RECOVERY_DISCARDED` 明确拒绝 encode/reconcile，不从损坏 SDI 猜归属；index footer 与原有 fail-closed 规则不变 |
...
| `dd.ddl/recovery` | `DictionaryDdlService`, `DictionaryDdlRecoveryService`, `DictionaryRecoveryIsolationPlanner`, recovery backup identity/manifest/service、catalog-loss manifest/snapshot/discovery | Production-wired blocking + object-force + catalog-loss v1 | recovery states、op8/9/10、raw DDL、可信 backup/import 和 exclusion planning 已接；全部 DDL 检查 clean fence并在稳定终点发布。仍无 online row log/binlog、跨实例 trust、主键/类型变更、foreign/generated、marker schema digest |
...
| `storage.api.ddl` + `storage.sdi` | storage schema/default/rewrite/rebuild DTO, `TableDdlStorageService`, `RecoveryBackupFile`, index descriptors, `DdlUndoMarker`, SDI repository/layout | Implemented v1 + blocking rebuild + recovery raw IO | create/drop/index/transfer/rebuild 之外，提供不读 page0 的 isolated move/delete 与 stable backup copy；write admission、SpaceId lease、handle/frame/path 约束均在 stable storage API 内，storage 不反向依赖 DD |
...
| --- | --- | --- |
| 公共 DD+storage+SQL Session+XA 组合根已接 | `DatabaseEngine` 先打开 catalog/manifest/XA registry，先规划对象级 force isolation，再从 DD 发现 recovery spaces；storage 以 registry 决议恢复 PREPARED，完成 recovery-object/普通 DDL 与 SDI reconcile 后构造 XA coordinator 与 Session | SQL v1 已含 comparison range、四隔离级别、persistent XA、受控 transfer、blocking ALTER 与 recovery export read-only；仍缺 online DDL、主键/类型/foreign/generated ALTER、network/prepared/optimizer/权限系统 |
