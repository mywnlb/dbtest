...
flowchart TD
  ...
  Truncate --> UndoCache["UndoCachedSegmentTruncationCoordinator"]
  UndoCache --> RsegPage3["RollbackSegmentHeaderRepository page3 v3"]
  UndoCache --> FSP
  Truncate --> Registry
  Truncate --> FSP
  Truncate --> BP
...
| UNDO truncate | `UndoTablespaceTruncationService.truncate` -> X lease -> `UndoCachedSegmentTruncationCoordinator.drainStableSpace` 先拒绝非空 persistent history，再非阻塞取得 cache gate、拒绝 active slots、每批最多 8 段同 MTR FSP drop + page3 top removal -> 空 inode -> marker -> `FlushService.flushThrough` -> `LruBufferPool.invalidateTablespace` -> `DataFileHandle.truncateTo` -> `UndoTablespaceFspRebuilder.rebuild` + page3 v3 format -> final state/Registry publish | Implemented (recovery production-wired；主动调用 test-wired)；同 epoch 可故障续作；已有 TRUNCATING marker 必须再次验证 page3 history/active/cache 与 FSP inode 均空；稳定文件已等于 initial size 时保持幂等 no-op；cache gate 忙不在 X 下等待 |
...
| `storage.api.undotruncate` lifecycle orchestration | `UndoTablespaceTruncationService`, `UndoTablespaceTruncationRecovery`, `UndoCachedSegmentTruncationCoordinator` | Implemented; recovery wired by `StorageEngine` E2 | 可恢复 UNDO 物理收缩与 cache owner 排空；E2 existing open 构造恢复参与者用于 TRUNCATING 续作；主动 truncate 仍待 purge/DML 调度 |
...
| Off-page LOB write/read/free | `StorageEngine.lobStorage` -> `LobCodec.logicalBytesForStorage` -> `DiskSpaceManager.reserveSpace(BLOB)` -> `requireSegmentPurposeForWrite(LOB)` -> `allocatePage(BLOB)` -> `LobPage.format`；read 逐页 S latch/release + chain/CRC；free 先校验再 FSP free + PAGE_INIT(ALLOCATED) | Implemented (0.21h)；TEXT/BLOB/JSON 记录 envelope 为 inline 或 `LobReference`+最多 32B prefix；页链经现有 FSP allocation/free、PAGE_INIT/PAGE_BYTES redo 与通用 recovery；写链单 MTR 原子但会同时 pin 全部新页。DML 自动 externalize、版本替换后的 purge 回收仍需 DD/LOB ownership 编排 |
...
| Undo record codec | `UndoRecordCodec` -> `TypeCodecRegistry.codecFor` per column -> `FieldWriter`/`FieldSlice` self-framing payload；UPDATE_ROW **和** DELETE_MARK 追加全量旧 image（旧隐藏列 + 全列）尾部 | Implemented (INSERT_ROW + UPDATE_ROW + DELETE_MARK，T1.3f)；0.21h external LOB old image 只保存稳定 reference/envelope，不复制完整 payload；单条 undo 自身仍不得跨 undo page |
...
| `api.lob` | `LobStorage`, `LobPage`, `LobPageLayout`, LOB exceptions | Implemented (explicit facade) | `StorageEngine` 生产持有；write/read/free 复用 BLOB reservation、LOB segment、MTR/WAL/recovery。未自动接入单聚簇 DML，因为当前无 DD 列到 LOB segment ownership 映射 |
...
| `storage.btree` split-capable | `SplitCapableBTreeIndexService`, node pointer types, `BTreeRedoDeltas`, `RecordPageStructureSnapshot` | Implemented (StorageEngine + DML facade + tests) | 任意高度 split、merge/root shrink、redistribute、全部读写 latch coupling、safe-node/SX restart 与 MTR ordering 均生产接线。0.21d 起统一 existing-page helper 在实际 level 驱动 leaf/internal schema-aware key 顺序校验，早于 child 选择、search 与写页。B+Tree redo：sibling link；internal/root 最终 after-image；恢复只 patch 页面，不重跑 SMO。仍缺 B-link/OLC、MVCC/segment 辅助页头、SQL/DD 多索引 |
...
flowchart TD
  ...
  UndoMgr -->|peek / reserve pop / commit 后 publish| Cache["UndoSegmentCacheDirectory (INSERT/UPDATE LIFO)"]
  ...
  Finalizer -->|drop ineligible/full segment| Allocator["UndoSpaceAllocator"]
  Finalizer -->|owner + history base CAS| RsegHeader["RollbackSegmentHeaderRepository page3 v3"]
  Finalizer -->|UPDATE first-page history link/unlink| Access
...
| Commit | `ClusteredDmlService.commit` -> `prepareCommit` 分配 no -> `UndoLogManager.onCommit`：有 UPDATE 时先 `HistoryList.beginAppend` 冻结 base -> `UndoSegmentFinalizer.finalizeCommit` 预检 page3/old-tail/new-node -> 一个 MTR 处理 INSERT cache/drop、page3 active/cache owner、history base、UPDATE old-tail/new-node links+COMMITTED 与唯一 terminal delta -> commit 后发布 slot/cache/history -> `TransactionManager.commit` -> durability -> release locks | Implemented for storage DML facade；纯 INSERT 不进 history；mixed 所有 owner/link/terminal 同批；transition lock 不跨 IO，物理前失败可释放，物理后失败保留 fail-stop fence；若 `onCommit` 失败事务保持 ACTIVE 且锁不释放 |
| Rollback (consume undo, T1.3d/1.4b/1.4c/1.6) | `ClusteredDmlService.rollback` -> `RollbackService.rollback` 预检 INSERT/UPDATE 两个 persistent logical head -> 按全局 `undoNo` 归并逆序执行短 MTR inverse -> 每次 marker commit 后发布所属 binding -> 双链 EMPTY 后 batch finalization 在同一 MTR 按段 drop 或 active→cache，并写一条 terminal delta | Implemented for live 与 recovery；recovery 按 creator 聚合最多一条 INSERT + 一条 UPDATE ACTIVE evidence 后走同一归并/批终结；marker 永不领先 inverse，物理边界后失败保持 fail-stop |
| Statement/savepoint rollback (1.4/1.4b/1.6) | `ClusteredDmlService.beginStatement` -> `RollbackService.createSavepoint` 快照 INSERT/UPDATE 双 head；失败 rollback 先分别预检两条局部链，再按全局 `undoNo` 归并执行 inverse -> 一个按 `PageId` 排序的 marker MTR 对变化的 first pages 做 CAS -> commit 后一次发布双 binding head | Implemented as explicit storage DML API；局部链物理高水位不回退且允许跨 kind 序号间隙，后续 append 不复用全局 undoNo；marker/CAS 失败不移动内存 head，Guard 标记 rollback-only；SQL/session 自动 lifecycle、命名 SAVEPOINT 与 savepoint 后锁精细释放未接 |
| Undo write (INSERT/UPDATE/DELETE) | `ClusteredDmlService.insert/update/delete` -> `UndoLogManager.planX`：已有 binding 冻结 append snapshot；首写优先冻结同 kind cached top/count，否则 ALLOCATE_NEW -> `UndoLogSegmentAccess.planRecord/plannedNewPages` 冻结编码、页数、reservation 与 redo workload -> facade begin 业务 MTR -> `appendPlanned` 重新校验；REUSE_CACHED 同 MTR page3 cache→active slot、首页激活、append，ALLOCATE_NEW 才建 FSP segment，已有 binding 直接 append -> 发布局部 head/全局 high-water -> 聚簇写 | Implemented；`UndoSegmentAcquisition` 精确区分三路；cached segment 复用后允许 grow/external，后续多页终结不再缓存；stale cache plan 在物理前拒绝 |
...
| Atomic slot release / segment finalization | `RollbackSegmentSlotManager.beginBatchFinalization` all-or-none 取 FINALIZING leases -> `UndoSegmentFinalizer` 短读 owner/kind/drop plan/history evidence -> 精确单 fragment 尝试 cache，其余 drop -> 动态预算 MTR 固定 FSP page0/page2→page3→普通 first pages 排序，原子修改 owner、history base/link 与 terminal delta -> commit 后发布 FREE/cache/history | Implemented for INSERT/mixed commit、live/recovery 双段 rollback、committed UPDATE purge；容量满/0/cache transition busy 直接 drop；history append/unlink 由独立有界 lease 串行；物理前失败恢复运行时 transition，物理后失败保留 fail-stop fence |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `storage.trx` facade | `TransactionManager`, `UndoLogManager`, `UndoWritePlan`, `UndoSegmentCacheDirectory`, `RollbackService`, `UndoSegmentFinalizer`, `ReadViewManager`, `MvccReader`, `PurgeCoordinator`, `HistoryList`, `PurgeSummary` | Implemented; production-held by `StorageEngine`; DML-wired by `ClusteredDmlService` | 生命周期门面 + immutable undo plan/精确 reservation/redo workload + bounded per-kind cache + undo 写/原子终结 + rollback progress + ReadView/MVCC + 单线程 purge；SQL/session/autocommit/命名 SAVEPOINT 尚未接 |
...
| `storage.trx` undo context | `UndoContext`, `UndoLogBinding`, `TransactionSavepoint`, `EmptyUndoBoundary` | Implemented; DML facade writes through `UndoLogManager` and drives boundaries through `DmlStatementGuard` | `lastUndoNo` 是事务全局且不回退的 append 高水位；INSERT/UPDATE binding 各持 slot/firstPage/local logical head/persistent snapshot；首写可从同 kind cache 生成新 binding；savepoint 快照双 head；modifiedTables 仍 deferred |
...
flowchart TD
  ...
  TrxUndo --> TrxWork["page3 v3 -> validate cache/FSP + reconcile slots + traverse persistent history -> ACTIVE rollback / RESUME_PURGE"]
...
| Flow | Current production chain | Current state |
| --- | --- | --- |
| Recovery orchestration | `StorageEngine.open(existing)` -> gate/doublewrite -> checkpoint/transaction baseline -> redo dispatcher -> boundary/undo truncate/space reconcile -> participant 读取 page3 v3；cached owners 短读首页/FSP，occupied owners 交 `RecoveredTransactionReconciler`；`PersistentHistoryRecovery` 从 base head 沿 next 一节点一短 MTR 校验 exact length、prev/tail/cycle、slot/state/identity，按物理顺序 restore history，并以 `max(reconciledNextNo,lastTransactionNo+1)` 恢复 counter -> ACTIVE rollback/RESUME_PURGE -> flush/force/open | Implemented production path；cache 只接受空单 fragment；orphan COMMITTED、linked ACTIVE、链遗漏/重复/高水位回退、page3/UNDO v1/v2 均 fail-closed；READ_ONLY_VALIDATE 不发布运行时状态；多索引/DD/XA/DDL_RECOVERY 未接 |
...
flowchart TD
  ...
  UndoMgr -->|peek/pop same-kind cached owner| Cache["UndoSegmentCacheDirectory"]
  Cache -->|persistent owner/history authority| Page3["RollbackSegmentHeaderRepository page3 v3"]
...
| Flow | Current production chain | Current state |
| --- | --- | --- |
| Undo segment acquire | `UndoLogManager.planX` 按 record 类型选择 INSERT/UPDATE binding；目标 kind 尚无 log 时优先 `UndoSegmentCacheDirectory.peek(kind)` 冻结 LIFO top/count，命中形成 `REUSE_CACHED`，否则 `ALLOCATE_NEW` -> 业务 MTR reserve active slot；reuse 原子 page3 cached top→active slot + `UndoPage.activateCached`，fresh 才精确 reserve/create FSP segment；随后 append、bind、attach | Implemented and production-reachable；同一事务最多一条 INSERT 与一条 UPDATE log；两类 cache 独立、固定容量、0 可禁用；reuse 不分配 root 页，stale top/count 在物理前失败；仍是单系统 undo space/单默认 rseg |
| Undo append | `UndoLogSegmentAccess.planRecord` -> `UndoRecordWritePlan` 冻结完整 codec bytes、fresh-root inline/external 决策、页数与 CRC -> `plannedNewPages` 算 root grow + payload 精确页数 -> execution `UndoLogSegment.appendPlanned` -> `preflightAppend` 校验 creator/index/物理高水位/persistent logical predecessor -> external 先写 payload 链再在普通 UNDO 槽发布 35B descriptor，inline 直接写旧编码 -> 构造 `RollPointer` -> 同一 MTR 更新 record count、物理 `LOG_LAST_UNDO_NO` 与 `UndoLogicalHead` | Implemented and production-reachable；inline 首字节/type bytes 不变；root 槽仍由 `UNDO_RECORD_PAYLOAD` redo，payload 页由 `PAGE_INIT(UNDO_PAYLOAD)+PAGE_BYTES` 保护，header count/high-water/head pair 由 metadata after-image 保护；物理高水位在 partial rollback 后不回退 |
...
| Cached page reset/activation | finalization `UndoLogSegment.resetForCache` 把单页段 header 置 `STATE_CACHED`、事务/commit/count/head/history links 清空（旧 record bytes 留在不可寻址区）；reuse `UndoLogSegmentAccess.activateCached` X-open 后逐字段/handle 校验，再置新事务 ACTIVE | Implemented；只以 header/freeOffset/count/head/link 为权威，不为缓存清零整页；恢复同时校验 first=last 与 FSP `used=fragment=1, extent=0` |
| Persistent history node mutation | commit `UndoLogSegmentAccess.appendHistoryNode` X-open old tail/new UPDATE first page（及 mixed cache reset pages）按 space/pageNo 排序，写 old.next/new.prev+next/state/commitNo；purge `unlinkHistoryHead` 写 newHead.prev=NULL，并 reset removed（cache）或配合 FSP drop | Implemented and production-reachable；全部证据先校验后写；page3 先于普通 first page，first pages 内全序；`UNDO_HISTORY_LINK_FIELD` redo 只覆盖 link 字段 |
...
| Transaction undo write (1.6 dual log) | `planInsert` 只读 INSERT binding/cache，`planUpdate/planDelete` 只读 UPDATE binding/cache -> immutable `UndoWritePlan(kind,acquisition,globalHighWater,targetSnapshot/cacheTop)` -> DML admission/begin -> `appendPlanned` -> record predecessor=同 kind 局部 head -> 同 MTR append/header update -> `UndoContext.publishAppend` 更新目标 binding head 与事务全局 `lastUndoNo` | Implemented and DML production-reachable；事务 undoNo 全局唯一递增；`ALLOCATE_NEW/REUSE_CACHED/APPEND_EXISTING` 三路预算不同；旧 `beforeX` API 已删除 |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `storage.undo` access | `UndoLogSegmentAccess`, `UndoLogSegment`, `UndoAppendSnapshot`, `UndoHistoryNodeSnapshot`, `UndoRecordWritePlan`, `UndoStoredRecordResolver`, `UndoPayloadStorage`, `UndoLogicalHead`, `UndoLog`, `UndoSegmentHandle`, allocation ports | Implemented; production-held by `StorageEngine` | create/open/plan/append、external chain、commit header、logical-head CAS、history node inspect/link/unlink、按 pointer 直读均生产可达；rollback/recovery/purge/MVCC 共用 resolver；`UndoLog` 单页 predecessor 与 `forEachRecord*` 仅测试/诊断 |
| `storage.undo` page | `UndoPageAccess`, `UndoPage`, `UndoPageLayout`, `UndoLogState`, `UndoPayloadPage`, `UndoPayloadPageLayout`, `UndoRedoDeltas` | Implemented | 普通 UNDO v3 flags gate + first-page 15-byte logical pair + history prev/next + record area=136 + every-page kind；external 页仍使用 `UNDO_PAYLOAD=9` 与 `UEP1/v1` body header；普通 v1/v2/未知版本 fail-closed |
| `storage.undo` rseg header | `RollbackSegmentHeaderRepository`, `RollbackSegmentHeaderLayout`, `RollbackSegmentHeaderSnapshot`, `RollbackSegmentHeaderCapacity`, `RollbackSegmentHistoryBase` | Implemented; production-wired by `StorageEngine` | 固定 **page3 v3**：head/tail/length/lastTransactionNo + active slots + INSERT/UPDATE cache arrays；fresh/rebuild、owner CAS、history append/remove、truncate/recovery 共用；head/tail 属 occupied slot，跨 active/cache owner 唯一；expected base/owner/count/top fail-closed；v1/v2/未知版本拒绝；free-list/多 rseg 未实现 |
| `storage.undo` record | `UndoRecord`, `UndoRecordCodec`, `UndoRecordType`, `UndoLogKind`, `UndoPayloadDescriptor` | Partial | 三类 record 全部编解码；INSERT log 仅允许 `INSERT_ROW`，UPDATE log 仅允许 `UPDATE_ROW/DELETE_MARK`，append/direct read/segment read/traversal 一致守门；inline 与 `0x7F/v1` external descriptor 编码不变；partial 仅指目标 `UPDATE_EXTERN_REF/DDL_MARKER` 等类型尚未实现 |
...
| undo/rollback/finalizer types | `StorageEngine` + `ClusteredDmlService` + formal recovery | 双 log append、双 head savepoint、按全局 undoNo merge 的 live/recovery rollback、batch slot lease 与多 segment atomic finalization；recovery terminal delta 每事务一次 | Wire automatic statement lifecycle / named SAVEPOINT；DD 多索引入口 |
...
| `PurgeCoordinator`(impl `PurgeTarget`) / `PurgeDriverWorker` / `HistoryList` / transition exceptions / history value types | `StorageEngine` production-wired（配置 `clusteredIndex` 时）；formal RESUME_PURGE 按持久链重建 | `HistoryList` 是 page3/first-page 双向链投影；有界 append/remove lease 不持锁跨 IO；purge 复核 creator 非 active、no boundary 与 live-view visibility，成功后 finalizer 原子 unlink+cache/drop 再 complete | 多 worker、二级索引与 DD-driven recovery；当前单 rseg blocked head 不越过 |

### recovery 层已由 Engine E2 接入

| Type | Current caller | Why it exists | Next action |
| --- | --- | --- | --- |
| recovery service/request/gate/progress + transaction recovery context/table/snapshot/reconciler/evidence + `PersistentHistoryRecovery` | `StorageEngine.open(existing)` + tests | doublewrite→redo + trx sink→undo resume/reconcile→page3/slot/cache cross-check→persistent history closure/high-water→rollback/purge→force/open；FORCE_SKIP/progress diagnostics 保持 | Session gate diagnostics；full discovery、DDL、多索引、XA/PREPARED 决议 |

### undo 层底层类型生产入口

| Type | Current caller | Why it exists | Next action |
| --- | --- | --- | --- |
| `DiskSpaceUndoAllocator` | `UndoLogManager` / `UndoLogSegment` main grow + external payload path + tests | Adapter implementing `UndoSpaceAllocator` port; bridges undo -> `DiskSpaceManager` including exact UNDO reservation | 已被 `UndoLogManager` 经端口调用；单聚簇 DML 的 plan/append 路径在页写前精确预留 root grow + payload 总页数 |
| `UndoLogSegmentAccess` / `UndoLogSegment` / `UndoRecordWritePlan` / `UndoStoredRecordResolver` / `UndoPayloadStorage` / `UndoLogicalHead` / history snapshot / page/record/codec/handle/allocation ports | `UndoLogManager` + `RollbackService` + `MvccReader` + recovery/purge production paths + tests | Multi-page root + external payload、v3 every-page kind/history link、cached reset/activation、persistent local head/history CAS、统一 resolver | 已生产接线；后续只需 DD 多索引、多 rseg/undo space、free-list 与未来格式迁移方案 |
...
| `UndoRecordType.DELETE_MARK` | `ClusteredDmlService.delete` -> `UndoLogManager.planDelete/appendPlanned`; plus codec/rollback/MVCC/purge | Undo record type for delete-mark | 已实现：只写 UPDATE log；物理移除归 purge |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| 0.21 Record 物理类型补齐 | 28 `TypeId` 已含 BIT/ENUM/SET/TEXT/BLOB/JSON family；时间类型仍只有 Record 归一化物理语义，JSON v1 为严格 UTF-8 text 而非 MySQL binary JSON | SQL binder/session 补 TIME 范围/时区、ENUM/SET 名称映射和 JSON binary/partial-update 时不得改变既有 Record stable encoding |
...
| explicit overflow chain 已接、自动 ownership 未接 | `LobStorage` 可显式 write/read/free，record/undo 可保存 external reference，redo/recovery/CRC/FSP reservation 已闭合；单聚簇 DML 不知道列对应哪个 LOB segment，也不会自动 externalize 或 purge 被版本替换的链 | 随 DD table/index metadata 建立 per-column/per-table LOB segment ownership，再在 DML/rollback/purge 编排引用发布与回收；禁止让 Record 反向依赖 FSP |
...
| prefix index 比较已接（0.13d/0.21c/0.21g/0.21h） | leaf/node-pointer 共用 `TypeCodec.compareKeyPart`；Unicode UTF-8 budget 会退到完整 code point；TEXT/BLOB external 只允许不超过记录中 32B inline prefix，JSON/full LOB 拒绝。**简化**：schema 的 prefix 仍以字节计，不是 MySQL 字符数 | 二级索引真正物化截断 key 与回表复核随 DD/secondary-index 接线；完整 UCA 字符权重另用新 stable id |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| 双链 rollback 消费者已接（1.6 dual log） | full/recovery 每步选择 INSERT/UPDATE 较大 head undoNo，inverse 与所属 marker 分离；partial inverse 全成后用一个按 PageId 排序的 marker MTR 更新受影响 heads；全回滚到双 EMPTY 后 batch finalization | SQL/session 自动 statement lifecycle、命名 SAVEPOINT、savepoint row-lock scope、多索引/DD 仍缺 |
| page3 v3 owner/history 目录、恢复分组与多段 atomic finalization 已接 | mixed transaction 占两个 active slots；终结同批处理 owner 与 history base/link；recovery 分开校验 active/cache 证据，再严格遍历 committed UPDATE 物理链；truncate drain/rebuild 覆盖 page3 且非空 history 拒绝 | 持久 free undo segment list、多 rseg/tablespace 选择另列后续 |
...
| 单线程聚簇 purge + **后台 driver + persistent recovery resume 已接** | purge 只接受物理 head 的 committed UPDATE；task 失败保留 owner，成功后原子 unlink+cache/drop；slot/cache/history 都在 MTR 后发布；no/visibility 任一不安全即停止，不越过 head | 多 worker / 二级索引 purge；blocked-head 调度优化需多 rseg |
...
| bounded cached undo segment reuse 已接 | INSERT/UPDATE 各自固定容量 LIFO；只有单 fragment 单页段可 cache，容量满/0/忙/多页均 drop；page3 v3 是 owner/history 恢复权威，重启校验 undo 首页与 FSP inode 后恢复；truncate 非阻塞 drain 并重建 page3 | 后续持久 free segment list 与多 rseg/tablespace 需要分区 cache directory、扩展选择策略；不在当前 `RollPointer` 中猜测 space/rseg |
