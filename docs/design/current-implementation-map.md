...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| 写路径 latch coupling 已接（insert/deleteClustered，0.13a），仍缺其余写算子/读路径/SX/版本重定位 | 自底向上 split、内部页 split、原地 root split、delete/purge 后 merge、原地 root shrink、redistribute、victim free page 已实现；**insert/deleteClustered 乐观 S-crab 下降 + 悲观回退已实现（0.13a）**；但 replace/setDeleteMark/purge/读路径仍全路径 latch，无 SX latch，无 root 版本重定位协议 | 0.13b 补 replace/mark/purge + 读路径 crab、SX latch、root/version retry、safe node 精判 |
