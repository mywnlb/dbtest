...
| 无 extern undo record payload（T1.3e） | 0.21h 起旧 LOB 值可在 UPDATE undo 中保存 external reference（不复制 payload），但一条 undo record 自身仍不能跨 undo page；普通超大旧行仍抛 `UndoPageOverflowException`；无改聚簇 PK | 独立 extern undo payload + 改 PK update 留后续片；不要与 LOB data chain ownership 混为同一回收协议 |
