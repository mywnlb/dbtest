...
flowchart TD
  ...
  Pool --> Inst["BufferPoolInstance shard (instanceLock + PageHashTable)"]
...
| Flow | Current production chain | Current state |
| --- | --- | --- |
| Page fix (INDEX) | `api.index.IndexPageAccess.openIndexPage` -> `MiniTransaction.getPage` -> `LruBufferPool.getPage`（facade 经 `BufferPoolRouter` 路由到归属 `BufferPoolInstance`）-> `instance.getPage` -> `acquire` (instanceLock: resident hit / 命中 LOADING 出锁等 `PageLoadFuture` / `obtainVictim` 装 LOADING 占位后**出 instanceLock** `readAndPublish` 读盘) -> release instanceLock -> `pageLatch.lock` -> `new PageGuard`(releaser=instance) -> `attachWriteListener(MtrRedoCollector)` + `memo.pushPageGuard` (`MiniTransaction.java:103-106`)；facade 再调 read-ahead 钩子 | Implemented；**0.10d 多 instance（默认 N=1，生产 N=1）**：facade 路由单页操作到分片、跨切面查询逐分片聚合；miss 读盘移出 instanceLock（Phase B：per-frame LOADING + load future）|
...
| Dirty page mark | `PageGuard.close` -> `FrameReleaser.release(frame, wrote)` -> `BufferPoolInstance.release`（归属分片，0.10d 起 `FrameReleaser` 由 instance 实现）OR-dirties via `markDirty` under `instanceLock`; sets `oldestModificationLsn`/`newestModificationLsn`/bumps `dirtyVersion` | Implemented；**E1 修 bug**：`markDirty` 改用 `oldestModificationLsn==null` 守卫（原 `!dirty`）——newPage 对驻留页重初始化先置 dirty=true（无 LSN），双 newPage 同 MTR（allocatePage+createIndexPage 同根页）后 commit markDirty 会因 dirty 已真而漏设 oldestMod，留 dirty+null oldestMod 帧致 flush/checkpoint NPE。flush-after-双newPage 之前潜伏（既有 btree 测试不 flush 未触发） |
| Eviction | `BufferPoolInstance.obtainVictim(cleanSkip)`（分片本地，无跨分片 stealing）优先 free/clean 帧直接复用；仅有脏 unfixed 帧时记 PageId、出 `instanceLock` 经 `DirtyVictimFlusher.flushVictim`（→`FlushCoordinator.singlePageFlush`：WAL gate+checksum+doublewrite+`completeFlush`）刷干净后回环重选；本轮 `cleanSkip` 防空转，无干净帧抛 `BufferPoolExhaustedException`。无 flusher 退回 legacy `writeBack` | Implemented；注入 flusher（生产 `StorageEngine`）后脏页淘汰 WAL 安全：redo 未 durable→`flushVictim` 返回 false→不写盘；`FAILED`→抛根因不吞。legacy no-flusher 路径字节级不变 |
...
| Package area | Representative classes | Current state | Notes |
| --- | --- | --- | --- |
| `storage.buf` pool core | `BufferPool`, `LruBufferPool`(facade), `BufferPoolInstance`, `PageHashTable`, `BufferPoolRouter`, `BufferFrame`, `PageGuard`, `PageLatchMode`, `DirtyVictimFlusher`, `BufferFrameState`, `FrameStateMachine`, `PageLoadFuture`, `BufferPoolLoadTimeoutException` | Implemented (production-wired) | **0.10d 多 instance 分片**：`LruBufferPool` 转 facade，经 `BufferPoolRouter`(`hash(PageId)%N` 确定路由) 把单页操作转发到归属 `BufferPoolInstance`、跨切面查询（dirty 候选合并按 oldestLSN 升序 / oldest LSN 全局 min / hasDirty / residentCount / residentCountInRange / 截断）逐分片聚合；每分片自有一把 `instanceLock` + `PageHashTable`(PageId→帧) + free list + LRU + `FrameStateMachine`，`FrameReleaser` 由 instance 实现。**并发不变量**：facade 跨切面操作一次只持一把 instance 锁（不同持两把）→ 无分片间锁序/死锁；分片间无 work stealing（某分片满即抛 `BufferPoolExhaustedException`）；`invalidateTablespace` 两阶段（全分片 drain+check 通过后再移除）保 all-or-nothing。容量按 base+前 r 个+1 切分（capacity≥N）。生产 N=1（`EngineConfig.bufferPoolInstanceCount` 默认 1，`StorageEngine` 经此构造）。**Phase B**：`FrameStateMachine`(FREE/LOADING/CLEAN/DIRTY/FLUSHING) + miss 读盘移出 instanceLock（LOADING 占位 + `PageLoadFuture` 有界等待）+ FLUSHING 与 dirty 正交。§13.1 per-instance 拆分锁（pageHash/freeList/lru 子锁）、DIRTY_PENDING/EVICTING/STALE 态、0.22 stale 校验 deferred |
...
| Clustered delete (T1.3d；0.12 merge；0.13a latch coupling) | `SplitCapableBTreeIndexService.deleteClustered(...)` -> **乐观** `tryOptimisticDelete`：`descendOptimistic`（内部层 S-crab、leaf X）-> `findEqual`（未命中/所有权不符=幂等 no-op）-> `deleteWouldUnderflow` 预判（同 `isUnderfull` 公式、freed 取上界偏保守）：不欠载则 `deleteMark`+`purge` **跳过 `reclaimAfterRemoval`**（仅 leaf 持 X）；欠载=unsafe **写页前**释放 leaf X 回退悲观 `descendPath`（全路径 X，全路径入 memo）-> `deleteInLeaf`（`findEqual`->所有权校验->`deleteMark`/`purge`->`reclaimAfterRemoval` 带 merge）-> `BTreeDeleteResult(removed, indexAfter, freedPages)` | Implemented (StorageEngine service root + `RollbackService` + tests); 幂等（未命中/不匹配=no-op）；不 import trx/undo；**0.12 起删成功触发 merge + 原地 root shrink + free page（仅悲观路径）**；**0.13a 乐观不欠载删除仅 leaf 持 X、跳过 merge** |
| Clustered purge (T1.3d；0.12 merge；0.13b latch coupling) | `SplitCapableBTreeIndexService.purgeDeleteMarkedClustered(...)` -> **乐观** `tryOptimisticPurge`：`descendOptimistic`（内部 S、leaf X）-> `findEqual` 严格校验（命中 + 仍 delete-marked + 隐藏列匹配，任一不符=stale no-op）-> `deleteWouldUnderflow` 预判：不欠载则 `purger.purge` **跳过 `reclaimAfterRemoval`**（仅 leaf 持 X）；欠载=unsafe 写页前释放 leaf X 回退悲观 `descendPath`+`purgeInLeaf`（带 merge）-> `BTreeDeleteResult(removed, indexAfter, freedPages)` | Implemented (StorageEngine service root + `PurgeCoordinator` + tests)；stale=no-op；与 delete 共用 0.12 欠载回收 + 0.13b 乐观预判 |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| miss 读盘已移出 `instanceLock`（Phase B 0.9）；多 instance 分片已落（0.10d） | per-frame LOADING 占位 + `PageLoadFuture` 有界等待：不同页 miss 并发读、同页只读一次、读失败清占位、命中 LOADING 超时/中断不悬挂；脏 victim 刷盘亦在锁外；仅帧表/LRU/state 短临界区在分片锁内。0.10d：facade 路由 + 每分片一把 `instanceLock`。**剩余**：legacy `flush`/`flushAll` 仍持锁直写（test/close-only）；读写并发期更细的 IO_FIX/EVICTING 态；§13.1 per-instance 拆分锁（pageHash/freeList/lru 子锁）未做 | legacy flush 路径统一/移除；按需细分 IO 状态；§13.1 拆分锁（0.10 后续） |
...
| 0.10 全落：read-ahead（linear+warmup+random）+ 多 instance 分片 + 专用 PageHashTable | 0.10a linear + 0.10b warmup + 0.10c random read-ahead（默认禁用）+ **0.10d 多 instance（facade+`BufferPoolRouter`+per-shard `BufferPoolInstance`/`PageHashTable`/`instanceLock`，默认 N=1）** 已落并生产接线（`StorageEngine` 经 `EngineConfig.bufferPoolInstanceCount`）；简化为单 instance 锁、单顺序流、random extent 驻留数启发式、warmup 同步预取 | §13.1 per-instance 拆分锁；0.22 stale-frame 版本校验；random 生产 config 开关；warmup IO 速率控制 |
| truncate/drop stale-frame 版本语义缺（0.22） | `invalidateTablespace` 可跨分片 drain/remove 单空间 frame（0.10d 两阶段 all-or-nothing），但 `PageHashTable`（0.10d 已抽出）尚无 stale validation——没有 `TablespaceVersion` / `SpaceLifecycleClock`、waiter recheck；drop/truncate 与并发 page admission 的陈旧帧边界仍靠调用纪律 | 0.22：引入空间生命周期版本，miss/load/admit/lookup/waiter 唤醒后统一复核，防止旧 frame 在 drop/truncate 后重新入表 |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| 写路径 latch coupling 全覆盖（全部聚簇写算子，0.13a/0.13b），仍缺读路径 crab / SX / 版本重定位 | **全部聚簇写算子（insert/deleteClustered/replaceClustered/setClusteredDeleteMark/purgeDeleteMarkedClustered）乐观 S-crab 下降 + 悲观回退（有欠载/溢出的算子）已实现（0.13a/0.13b）**；但读路径（lookup/scan）仍全路径 S，无 SX latch，无 root 版本重定位协议 | 0.13c 补读路径 crab；0.13d 补 SX latch、root/version retry、safe node 精判 |
