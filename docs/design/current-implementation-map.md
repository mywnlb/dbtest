...
| FSP/Undo/B+Tree/Trx 逻辑 redo 已替代或补充对应物理 bytes | 0.23a/0.23b 已闭合 MTR 纪律；0.19b–0.19i 已接 FSP/undo/B+Tree/trx delta；提交视图精确过滤被 after-image 覆盖的物理 bytes；0.20b/0.20c 增加 LogBlock 保护和 operation budget 精确结算 | page0 信封、lifecycle marker、record row bytes、B+Tree node/root 结构 bytes 等未被逻辑 delta 覆盖的物理字节仍走 `PAGE_BYTES`；下一步按收益细化 B+Tree delta |
...
| Gap | Current consequence | Preferred resolution |
| --- | --- | --- |
| 0.19 逻辑 redo + trx recovery 消费、0.20b/0.20c LogBlock 与 operation budget 已闭合 | page-local delta 可独立 replay；`TRX_STATE_DELTA` 合并 checkpoint baseline/page3；固定 block、连续性校验、显式预算与 actual 结算共同保护输入 | B+Tree node/root 更细结构 redo；按领域 plan snapshot 收紧 profile |
