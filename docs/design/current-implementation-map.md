...
| Rollback (consume undo, T1.3d/1.4b/1.4c) | `ClusteredDmlService.rollback` -> `RollbackService.rollback(txn, clusteredIndex)` -> 从当前 `UndoContext.logicalHead` 逐 pointer 短 MTR 预检剩余链；ACTIVE 通过后才 `txnMgr.beginRollback`，ROLLING_BACK 保持状态并从已发布进度重试 -> 每条短读 current/真实 predecessor并释放 undo latch/fix -> 独立 index MTR inverse commit -> 独立 marker MTR CAS 后退持久头 -> 发布内存进度 -> 到 EMPTY 后 `UndoSegmentFinalizer` 独占 FINALIZING lease，同一 MTR drop segment + CAS 清 page3 + `TRX_STATE_DELTA(ROLLED_BACK)`，commit 后 complete lease -> `txnMgr.finishRollback` -> facade 仅在事务已 ROLLED_BACK 时 `LockManager.releaseAll` | Implemented for storage DML facade；恢复回滚到 EMPTY 也走同一 finalizer（无 live trx delta）；marker 永不领先 inverse；非终态失败保留 ROLLING_BACK、slot 与 row locks；重复终结在 page/FSP 前拒绝，物理写开始后的异常保留 FINALIZING 并抛 fail-stop `UndoFinalizationException`；纯状态 `TransactionManager.rollback()` 仍供只读/未写事务 |
...
| Atomic slot release / segment finalization | `RollbackSegmentSlotManager.beginFinalization(slot,expectedOwner)` 短锁 `ACTIVE -> FINALIZING` -> `UndoSegmentFinalizer.prepare` 短读 page3→undo first page，校验 identity 并取 handle -> lease 标记 physical mutation started -> final MTR `dropUndoSegment`（FSP page0/page2）-> `clearSlot` owner CAS（page3）-> 可选 trx delta -> commit -> lease `complete` 发布 FREE；purge 再 `HistoryList.completeCommitted(expected)` | Implemented for INSERT commit、live rollback、recovery rollback、committed purge；重复/stale 命令在 page/FSP 前拒绝；预检失败 lease 恢复 ACTIVE，物理边界后异常与 commit 后 crash 均保留 FINALIZING；FSP metadata 与 RSEG_SLOT clear 同一 redo batch，page3 是重启权威；持久 history linked-list/正式 trx recovery table 未实现 |
...
flowchart TD
  ...
  TrxUndo --> TrxWork["page3 scan -> ACTIVE: current/predecessor short reads -> inverse + per-record marker / COMMITTED history rebuild"]
...
| Flow | Current production chain | Current state |
| --- | --- | --- |
| Recovery orchestration | `StorageEngine.open(existing)` -> `DiskSpaceManager.openTablespaceForRecovery(undo + non-skipped EngineConfig.recoveryTablespaces)` -> `CrashRecoveryService.recover(request)` -> gate/doublewrite/checkpoint/redo replay/boundary install/undo truncate resume/space reconcile -> `TransactionUndoRecoveryParticipant.recoverAfterRedo`：page3 扫描 ACTIVE/COMMITTED；ACTIVE 经 `RollbackService.rollbackRecovered` 读 first-page persistent `UndoLogicalHead`，每条分别短读 current/真实 predecessor、释放 undo 页后独立 index MTR 幂等 inverse，再以独立 marker MTR CAS 后退持久头；COMMITTED 按 `COMMIT_NO` 重建 history/counter -> flush recovery 新 redo -> `PageStore.forceAll` -> open traffic/workers | Implemented production path；每条 marker 已持久则下次从 predecessor/EMPTY 继续，marker 落后则安全重做幂等 inverse；链首 pair、严格下降、segment/transaction/index 归属损坏均在对应 inverse 前 fail-closed；重复恢复 EMPTY head 不再执行 inverse；NORMAL 仍为 9 formal enum stages，READ_ONLY_VALIDATE/FORCE_SKIP 语义不变；多索引/DD/prepared/DDL_RECOVERY 未接 |
...
| `UndoLogManager` / `UndoContext` / `TransactionSavepoint` / `EmptyUndoBoundary` / `RollbackSegmentSlotManager` / `RollbackService` / `UndoSegmentFinalizer` / `RollbackSummary` | `StorageEngine` production-held; `ClusteredDmlService` writes beforeX, commits/full-rolls back, and returns `DmlStatementGuard` | undo append/persistent logical head、精确 savepoint、可重试 full/recovery rollback，以及四条路径共享的 atomic segment/page3 finalization 均生产可达 | Wire automatic statement abort / named SQL SAVEPOINT；补 savepoint lock scope、正式 trx recovery table 与 DD 多索引入口 |
...
| rollback 消费 undo 已接（T1.3d/e/f + 1.4/1.4b/1.4c + atomic finalization） | live full/recovery rollback 共用 current/predecessor 短读；每条 inverse 后 marker CAS 后退持久头，live 再发布内存 pair。到 EMPTY 后 finalizer 先独占 FINALIZING，再同批 drop segment + page3 owner clear；live 同批追加 rollback delta，MTR commit 后 complete 发布 FREE。partial rollback 仍只退逻辑头、不终结段 | per-record marker 与最终 drop/clear crash 边界已闭合；剩余是 DD 多索引、PREPARED/RECOVERED_ACTIVE、正式 trx recovery table；trx state diagnostic redo 仍非恢复权威 |
...
| Trx state diagnostic redo 已接但未重建事务表 | update/delete commit MTR 与纯 insert finalization MTR 追加 `TRX_STATE_DELTA(COMMITTED)`；live rollback finalization MTR 追加 `ROLLED_BACK`；recovery rollback 无 live transaction delta；handler 仍 no-op 消费 | page3 occupied slots 是当前恢复权威；已清 slot 的纯 insert 提交号不会被 page3 counter scan 看见，后续正式 trx recovery table 必须消费持久事务状态并支持 prepared/recovered-active |
...
| slot 目录、恢复重建与四路 atomic finalization 已接 | claim 先 RESERVED、短 S-latch 预检 page3、再分配/bind/persist；INSERT commit、live/recovery rollback、committed purge 均先独占 FINALIZING lease，再同一 MTR 执行 FSP drop + `RSEG_SLOT` CAS clear，可选 trx delta同批，commit 后 complete；恢复只扫描仍占用槽并重建 ACTIVE，运行期 RESERVED/FINALIZING 不落盘 | truncate rebuild 尚未重格式化 page3；多 rseg/history 持久链表、extern payload 另列后续；正式 trx recovery table 仍需解决已清 slot 后的 transaction counter/state 证据 |
