...
| DDL log / undo marker v4 | `DictionaryDdlService`按operation构造source/intermediate/target canonical schema digest、execution protocol与`OPEN` control -> `PersistentDdlLogRepository.prepare`（`PersistentDdlLogRepository.java:62`）校验checkpoint策略 -> catalog `DDL_LOG(7)`；后续只允许expected-phase或同phase单字段control/fence CAS，identity/protocol/digest不可改；v1-v3由decoder映射`LEGACY_PHASE_ONLY` | Implemented for operation 1..11（含`ALTER_TABLE_INPLACE`）；SHA-256 digest覆盖schema/table/column/index/options/row-format与逻辑LOB capability，物理binding/path由marker、descriptor和exact-CAS独立证明。operation-specific恢复在采用DD/SDI/descriptor前复算；cancel-capable协议只允许`PREPARED: OPEN→CANCEL_REQUESTED|FORWARD_ONLY`，retirement fence只允许先在`PREPARED/OPEN`冻结再参与方向CAS；cancellation与fence均有固定shape/单调约束。legacy格式仍可恢复但不能由v4 encoder产生或冒充digest verified；temporary undo未接 |
...
| `dd.ddl/recovery` | `DictionaryDdlService`, `DictionaryDdlRecoveryService`, schema digest/control/registry facade, `OnlineAlterStrategySelector`, `OnlineAlterRuntime`, `OnlineAlterRecoveryRuntime`, index/alter retirement barriers、recovery backup identity/manifest/service、catalog-loss manifest/snapshot/discovery | Production-wired marker v4 + online ADD/DROP + general INPLACE/SHADOW ALTER + object-force/catalog-loss v1 | live/recovery共用gate、journal、tracker与retirement语义；通用恢复逐字段核对marker/header/manifest/DD/SDI/descriptor/fence，source+OPEN/CANCEL回滚，source+FORWARD_ONLY只在READY+RECONCILED后前滚，target继续retirement。复制binlog、跨实例trust、主键/任意类型/foreign/generated仍未实现 |
...
| `PurgeCoordinator` / `PurgeWorkerPool` / `PurgeDriverWorker` / `HistoryList` / `HistoryTablePurgeBarrier` / purge safety/guard types | `StorageEngine` production-wired；recovery、DD DROP 与 DML 共用 | persistent history projection、affected-table completion DAG、version-safe secondary-first multi-worker、strict head finalization、real RESUME_PURGE 与 DROP barrier 已闭环；普通 close 先同时 request driver/pool，再共享 deadline await | 多 rseg/undo-space、跨 rseg blocked-head 选择、purge→truncate 自动调度；index/page 内并行与自适应 IO 限速 deferred |
...
| 有界 multi-worker table-aware purge + 后台 driver + real recovery resume 已接 | 默认 4 worker/16-log window；同表 completion DAG，异表并行；task 失败保留 COMMITTED owner/history/table barrier；finalization 后才摘 history并唤醒 DROP；timeout fail-stop且不在线程位于记录内 MTR 时强制中断 | 跨 rseg blocked-head 调度需多 rseg；purge→truncate 自动调度未接 |
