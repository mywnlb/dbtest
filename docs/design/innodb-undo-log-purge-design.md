...
6. MTR commit 后才发布 slot/reuse directory/history；失败前不移动内存 head，越过物理边界失败进入 fail-stop fence。purge→truncate 自动调度尚未接入。
...
当前实现落点（2026-07-17）：`storage.api.ddl.DdlUndoMarker` 已由 CREATE/DROP TABLE 的独立 catalog DDL log
